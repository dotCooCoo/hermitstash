...
module.exports = function (app) {
  ...
  app.post("/admin/users/invite", idempotency, async (req, res) => {
    ...
    try {
      ...
      var role = check.role;
      if (usersRepo.findByEmail(email)) throw new ValidationError("Email already registered.");

      // Check for existing pending invite
      var existing = invitesRepo.findAll({}).filter(function (i) { return i.email === email && i.status === "pending"; });
      if (existing.length > 0) throw new ValidationError("Invite already sent to this email.");
      ...
      invitesRepo.create({
        ...
        status: "pending",
        expiresAt: new Date(Date.now() + 2 * C.TIME.days(1)).toISOString(),
  ...
  app.post("/admin/users/invite/resend", async (req, res) => {
    ...
    try {
      ...
      if (!pending.length) throw new NotFoundError("No pending invite for this email.");
      ...
      throw new AppError("Failed to resend invite.", 500);
    }
  });
  ...
  app.post("/admin/users/:id/password-reset-link", async (req, res) => {
    if (!requireAdmin(req, res)) return;
    try {
      if (!config.localAuth) throw new ForbiddenError("Local auth is disabled.");
      var user = usersRepo.findById(req.params.id);
      if (!user) throw new NotFoundError("User not found.");
      if (user.authType !== "local" || !user.passwordHash) throw new ValidationError("User has no local password to reset.");
      ...
      verificationTokensRepo.create({
        ...
        type: "password_reset",
        expiresAt: new Date(Date.now() + C.TIME.hours(1)).toISOString(),
        createdAt: new Date().toISOString(),
      });
      ...
      res.json({ success: true, resetUrl: resetUrl, expiresAt: new Date(Date.now() + C.TIME.hours(1)).toISOString() });
    } catch (e) {
      if (e.isAppError) throw e;
  ...
  app.post("/admin/users/:id/resend-verification", async (req, res) => {
    ...
    try {
      var user = usersRepo.findById(req.params.id);
      if (!user) throw new NotFoundError("User not found.");
      if (user.status !== "pending") throw new ValidationError("User is not awaiting email verification.");
      ...
      verificationTokensRepo.create({
        ...
        token: b.crypto.sha3Hash(rawToken),
        type: "email",
        expiresAt: new Date(Date.now() + C.TIME.days(1)).toISOString(),
        createdAt: new Date().toISOString(),
      });
  ...
  app.post("/auth/invite/accept", rateLimit.guard({ max: 10, windowMs: C.TIME.minutes(15), algorithm: "fixed-window" }), async (req, res) => {
    try {
      ...
      if (!token || !displayName) throw new ValidationError("Name is required.");
      ...
      var invite = invitesRepo.findAll({}).filter(function (i) { return i.tokenHash && i.tokenHash.length === tokenHash.length && b.crypto.timingSafeEqual(i.tokenHash, tokenHash) && i.status === "pending"; })[0];
      if (!invite) throw new ValidationError("Invalid or expired invite.");
  ...
  app.post("/admin/users/:id/suspend", async (req, res) => {
    ...
    if (target.role === "admin") {
      var adminCount = usersRepo.count({ role: "admin", status: "active" });
      if (adminCount <= 1) {
        throw new ValidationError("Cannot suspend the last admin.");
      }
    }
