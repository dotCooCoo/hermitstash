 ...
 * HermitStash web UI when enforceMtls is enabled. They're signed by the separate
 * classical browser CA (lib/mtls-ca-browser.js, ECDSA-P384-SHA384 — not the sync
 * CA, which may be post-quantum ML-DSA-87 that browsers can't yet use), packaged
 * as PKCS#12 for one-click install in browsers / OS cert stores, and tracked as
 * rows in the api_keys table so the existing revoke flow cascades to
 * cert_revocations.
 *
...
// Browser certs are signed by the SEPARATE classical (ECDSA-P384-SHA384) browser
// CA — not the sync CA (which may migrate to ML-DSA-87, unusable in browsers).
  ...
  app.post("/admin/browser-certs/generate", async function (req, res) {
    ...
    try {
      ...
      if (!mtlsCaBrowser.exists()) {
        // Ensure the browser CA exists before proceeding — the first browser cert
        // on a fresh install boots this classical CA (independent of the sync CA).
        await mtlsCaBrowser.initCA();
      }

      var result = await mtlsCaBrowser.generateClientP12({ cn: cnRaw, password: password, validityDays: validityDays });
