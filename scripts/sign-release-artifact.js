...
"use strict";
/**
 * Sign a release artifact (the docker-save tarball of the published
 * image) with the project's ML-DSA-65 release-signing key, then
 * write the signature alongside as `<artifact>.mldsa.sig`.
 *
 * Invoked from `.github/workflows/docker-publish.yml`:
 *
 *   node scripts/sign-release-artifact.js artifacts/hermitstash-vX.Y.Z.image.tar
 *
 ...
 * stale / wrong env secret silently producing unverifiable
 * signatures.
 */
...
function main() {
  ...
  if (!secB64 || secB64.length === 0) {
    fail("RELEASE_PQC_SIGNING_KEY env not set. Run scripts/generate-release-signing-key.js once + set the secret in the release environment.");
  }
  ...
  // Self-verify against the in-tree pubkey before writing. If verify
  // fails, refuse — the operator-side verification path would also
  // fail and the workflow's gh release create would attach a sig that
  // no one can verify.
