...
var PATTERNS_NA = {
  ...
  "policy-compared-against-the-literal-audit": "The detector targets code that BRANCHES on a policy value, which is guard-implementation surface. HS only ever SETS policies and hands them to a guard; it compares nothing against \"audit\" or \"audit-only\" anywhere outside the vendored tree, so the audit/audit-only divergence it exists to catch has no site here. Re-check if HS ever reads a policy back and acts on it.",
  "test-masking-finally-around-handle-drain": "N/A as written: the detector anchors on `\\w*drainOpenHandles(` inside a `finally`, and HS's tests call no such helper — the only occurrences of that name in this tree are inside tests/lint/blamejs-codebase-patterns.snapshot.js, which is the copy of blamejs's own gate. The DEFECT it describes is not N/A: a `finally` whose teardown can throw replaces the body's error, so the assertion that failed is lost and the teardown fault is all that gets reported. HS has that shape in a handful of test files (unguarded server.close / db.close / fs.rmSync), tracked separately rather than by porting a detector keyed to a helper HS does not have.",
  ...
  "domain-labels-must-not-drop-empty-labels": "the detector targets code that decomposes a hostname into labels, and HS decomposes none. Its only split(\".\") calls are on a filename's extension (app/shared/sanitize-filename.js) and on process.versions.node (server.js) — neither is a domain. Host identity is resolved through b.publicSuffix.canonicalDomain and b.ssrfGuard.canonicalizeHost, which own the empty-label rule. Re-check if HS ever splits a hostname itself",
  "gate-context-rebind-must-not-flatten-the-chain": "blamejs gate-context internals. HS builds no gate context and rebinds none — it reads b.gateContract.CHAR_THREATS_REJECT_ALL as a frozen constant and spreads it. Every \"rebind\" in HS names DNS rebinding in the SSRF and webhook paths, which is an unrelated sense of the word",
