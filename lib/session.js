...
// Ensure the session DB exists with owner-only mode on Unix (best-effort —
// Windows governs permissions by ACL). sessionDbPath sits in a world-writable,
// predictable tmpfs dir (/dev/shm), so the create must refuse a symlink planted
// at that path: openAppendNoFollowSync opens O_WRONLY|O_APPEND|O_CREAT|O_NOFOLLOW,
// so a planted symlink fails the open (ELOOP) instead of redirecting the create.
try {
...
// Cookie value is the framework-emitted token verbatim, a vault:-prefixed
// envelope, so its shape matches the framework's. A cookie issued before
// v1.9.30 carries no prefix and fails b.session.verify on the first request
// after an upgrade; those users re-authenticate once.
//
// `Secure` follows the REQUEST's own scheme, peer-gated via clientIp so a
// forwarded header counts only from a declared trusted proxy. Deriving it from
// config.rpOrigin instead reads the scheme the operator NAMED rather than the
// one the browser USED: with an https rpOrigin — what passkeys require — a
// plain-HTTP visitor is sent a Secure cookie, which the browser discards, so
// every following request arrives unauthenticated with nothing to show for it.
...
async function sessionMiddleware(req, res, next) {
  try {
    ...
    res.end = function (chunk, encoding, callback) {
      ...
      // Always flush mutated session data (CSRF/OAuth/passkey state). The
      // lastActivity/expiresAt bump (b.session.touch) is the idle-clock reset —
      // a route that opts out via req._skipActivityUpdate (e.g. the
      // /auth/session-check liveness heartbeat) must NOT advance it, or the
      // 60s heartbeat would refresh the clock forever and the idle timeout
      // could never fire.
      var ops = [b.session.updateData(token, req.session).catch(function (_e) { /* persist best-effort */ })];
      if (!req._skipActivityUpdate) {
        // Thread HS's configured idle + absolute floors so touch() enforces the
        // SAME window verify() does (b.session.touch re-checks the floors on
        // refresh). Without them, touch falls back to the framework defaults
        // (idle 30m, absolute 12h) and can delete a session HS's verify would
        // still accept, or extend one it would have expired.
...
async function clearSessionsForUser(userId) {
  if (!userId) return 0;
  // b.session.destroyAllForUser DELETEs the store-backed session rows and raises the
  // per-subject stateless "valid-from" boundary. As of blamejs 0.15.16 that boundary
  // write falls back to the configured session store when no framework DB is
  // initialized (HS's model — own DB lifecycle, store-verified sessions, no
  // b.db.init), so it resolves cleanly instead of throwing db/not-initialized. Any
  // error now means the revocation itself failed and MUST propagate — fail closed
  // rather than report a partial/failed "log out everywhere" / suspend / delete as 0.
...
// drop its client-side state. b.session.logout emits a W3C Clear-Site-Data
// header (cookies + storage + cache + executionContexts). Clear-Site-Data is
// a W3C WebAppSec specification, not an RFC — there is no IETF document for it.
...
// `req` decides whether the expiry cookie carries Secure. Without it the
// framework answers with the secure default, and a browser drops a Secure
// cookie that arrives over plain HTTP — so on a cleartext deployment the header
// is discarded and the cookie this call exists to clear stays in the jar. The
// session row is destroyed either way, so the survivor is a dead cookie rather
// than a live session, but it is sent on every later request to an origin that
// has no session for it.
//
// The answer comes from clientIp rather than from handing the framework `req`.
// Both resolve the scheme peer-gated, but the framework's copy is not given
// HS's TRUST_PROXY list, so behind a TLS-terminating proxy it reads the socket,
// sees cleartext, and answers false — while _setSessionCookie, which does know
// the list, answers true for that same request. Deleting works either way, since
// a cookie is keyed by name, domain and path and not by Secure; passing the
// resolved value keeps the expiry cookie and the cookie it expires agreeing
// about the scheme instead of differing for a reason nothing states.
//
// COOKIE_NAME carries no `__Host-`/`__Secure-` prefix, so an explicit false is
// not the contradiction the framework refuses for those names.
...
// Release the session store's file handle.
//
// The store is opened at module load and, until now, never closed — not at
// shutdown and not by anything else. On Windows an open handle blocks removal of
// the file and of the directory holding it, so a process that used sessions left
// its store directory behind on exit.
//
// Safe to call more than once, and safe to call on a store that was never used.
function closeStore() {
  try {
