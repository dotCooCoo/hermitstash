...
function sessionMiddleware(req, res, next) {
    ...
    if (sid) {
      ...
      if (stored && Date.now() < stored.expires) {
        ...
        // Using /24 instead of exact IP avoids false logouts on mobile/corporate NAT
                   ...
                   (stored.data._ua !== ua || ipSubnet(stored.data._ip) !== ipSubnet(ipAddr))) {
          sessions.delete(sid);
          stored = null;
        } else {
          session = stored;
  ...
  if (!session) {
    sid = generateId();
    session = { data: { _ua: ua, _ip: ipAddr }, expires: now + MAX_AGE, lastActivity: now };
...
module.exports = {
  ...
  // Lookup by session ID — reads directly from the session store and
  // returns the unsealed data. Exposed for tests that need to inspect
  // server-side session state (e.g. CSRF token for logout flows).
  getSessionData: function (sid) {
    var row = stmtGet.get(sid);
    if (!row) return null;
    var json = tryUnseal(row.data);
    if (!json) return null;
