...
const { VAULT_PREFIX } = require("./constants");
...
var _tmpDir = process.env.HERMITSTASH_TMPDIR || (require("fs").existsSync("/dev/shm") ? "/dev/shm" : path.join(__dirname, "..", "data"));
...
// Encrypt session ID via ML-KEM-768 + AES-256-GCM (post-quantum)
function sealCookie(sid) {
  return vault.seal(sid).substring(VAULT_PREFIX.length);
}

// Decrypt session ID via ML-KEM-768 + AES-256-GCM (post-quantum)
...
function sessionMiddleware(req, res, next) {
  ...
  if (cookies[COOKIE_NAME]) {
    ...
    if (sid) {
      ...
      if (stored && Date.now() < stored.expires) {
        ...
        var idleTimeout = config.sessionIdleTimeout || 1800000;
  ...
  res.writeHead = function (statusCode, ...rest) {
    ...
    sessions.set(sid, session);
    const sealed = sealCookie(sid);
    const secure = config.rpOrigin && config.rpOrigin.startsWith("https") ? "; Secure" : "";
    const cookie = `${COOKIE_NAME}=${encodeURIComponent(sealed)}; Path=/; HttpOnly; SameSite=Lax${secure}; Max-Age=${MAX_AGE / 1000}`;
