...
var { TIME, VAULT_PREFIX, DATA_DIR: _dataDir } = require("./constants");

var COOKIE_NAME      = "hs_sid";
var MAX_AGE          = TIME.SEVEN_DAYS;
var DEFAULT_IDLE_MS  = TIME.THIRTY_MIN;

// Resolve tmpfs preference for the sessions DB.
var _tmpDir = process.env.HERMITSTASH_TMPDIR
...
function _stripCookieToken(token) {
  return token.startsWith(VAULT_PREFIX) ? token.substring(VAULT_PREFIX.length) : token;
}

function _restoreCookieToken(cookieValue) {
  return cookieValue.startsWith(VAULT_PREFIX) ? cookieValue : VAULT_PREFIX + cookieValue;
}

function _anonUserId() {
  return "anon:" + b.crypto.generateToken(16);
}

function _setSessionCookie(res, token) {
  var stripped = _stripCookieToken(token);
  var secure = config.rpOrigin && config.rpOrigin.startsWith("https") ? "; Secure" : "";
  var cookie = COOKIE_NAME + "=" + encodeURIComponent(stripped)
             + "; Path=/; HttpOnly; SameSite=Lax" + secure
             + "; Max-Age=" + (MAX_AGE / 1000);
...
async function sessionMiddleware(req, res, next) {
  try {
    ...
    var fingerprintFields = ["clientIpPrefix", "userAgent"];
    ...
    if (cookies[COOKIE_NAME]) {
      var rawCookie = decodeURIComponent(cookies[COOKIE_NAME]);
      var candidateToken = _restoreCookieToken(rawCookie);
      var verified = await b.session.verify(candidateToken, {
        req: req,
        idleTimeoutMs: idleTimeoutMs,
        absoluteTimeoutMs: 0,                  // disabled — TTL alone bounds lifetime
