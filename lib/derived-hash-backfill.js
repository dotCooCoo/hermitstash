...
function run(opts) {
  ...
  for (var ti = 0; ti < tables.length; ti++) {
    ...
    // Wrap each table's read+re-key in one write transaction so a concurrent
    // write to a sealed source between the page read and the UPDATE can't leave
    // a stale keyed digest (index corruption). BEGIN IMMEDIATE takes the write
    // lock up front; the dual-read in field-crypto.lookupHash still masks any
    // in-flight state until every server has run the backfill.
    if (!dryRun) db.rawExec("BEGIN IMMEDIATE");
    try {
    for (;;) {
    ...
    if (!dryRun) db.rawExec("COMMIT");
    } catch (_txe) {
      if (!dryRun) { try { db.rawExec("ROLLBACK"); } catch (_re) { /* no active tx — nothing to undo */ } }
      throw _txe;
    }
    if (tableRewritten > 0) {
