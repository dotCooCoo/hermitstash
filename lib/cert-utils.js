...
// Lazy-load db to avoid circular dep (db → field-crypto → audit may
// pull cert-utils on revocation events).
 ...
 * @param {string} certSha3Fp — canonical SHA3-512-of-PEM digest from
 *   certFingerprintSha3(), NOT Node's cert.fingerprint256 (SHA-256-of-DER)
 * @returns {string} SHA3-512 hash suitable for DB lookup
 */
function hashCertFingerprint(certSha3Fp) {
  return b.crypto.namespaceHash(HASH_PREFIX.CERT_FP, certSha3Fp || "");
}
 ...
 */
function isCertRevoked(certSha3Fp) {
  var db = dbLazy();
  var fpHash = hashCertFingerprint(certSha3Fp);
 ...
 */
function revokeCert(certSha3Fp, cn, reason) {
  if (!certSha3Fp) return false;
  var db = dbLazy();
  var fpHash = hashCertFingerprint(certSha3Fp);
...
/**
 * Check whether a TLS peer certificate has been revoked. Reconstructs the
 * canonical SHA3-512 fingerprint from the peer's DER bytes (peerCert.raw) — the
 * SAME value revokeCert stores (an API key's certFingerprint is
 * certFingerprintSha3(pem)). The previous direct isCertRevoked(peerCert.fingerprint256)
 * hashed Node's SHA-256-of-DER, which never matched the stored SHA3-of-PEM, so
 * revocation was a silent no-op — every revoked cert still authenticated.
 * @param {object} peerCert — TLS peer cert (must carry the `.raw` DER buffer)
 * @returns {boolean} true if revoked
 */
function isPeerCertRevoked(peerCert) {
  if (!peerCert || !peerCert.raw) return false;
  var pem = "-----BEGIN CERTIFICATE-----\n" + peerCert.raw.toString("base64") + "\n-----END CERTIFICATE-----";
  return isCertRevoked(certFingerprintSha3(pem));
}
 ...
 * the source of a PEM (the @blamejs/pki mTLS engine, openssl, on-disk
...
function canonicalCertPem(pem) {
  ...
  // A non-empty body whose base64 decodes to zero bytes (e.g. body = "=") yields
  // derB64 = "" → "".match(/.{1,64}/g) === null → a TypeError on .join(). Guard it
  // the same way as the empty-body case above so a malformed cert produces a stable
  // empty canonical form (which hashes deterministically and never matches a
  // revocation row) rather than throwing on the fingerprint / revocation path.
  if (!derB64) return "";
...
function generateEnrollmentCode() {
  var codeBytes = b.crypto.generateBytes(C.BYTES.bytes(8));
...
module.exports = { hashCertFingerprint, isCertRevoked, isPeerCertRevoked, revokeCert, generateEnrollmentCode, canonicalCertPem, certFingerprintSha3 };
