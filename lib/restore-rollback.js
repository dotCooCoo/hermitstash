...
var { TLS_FILES } = require("./backup-crypto");
...
// hermitstash.db.enc, then vault.key(.sealed), then the TLS/mTLS-CA material
// (step 5b). We roll back in any order because each .pre-restore is
// independent. The TLS_FILES locals (tls/fullchain.pem, tls/privkey.pem(.sealed),
// ca.key(.sealed), ca.crt) are included so a failed restore reverts cert
// material symmetrically with the DB/vault — otherwise a throw reaching the
// outer catch after step 5b overwrote the live CA leaves the reverted DB
// (cert_revocations / credentials referencing the old CA) pointing at a
// divergent on-disk CA pair, breaking the mTLS trust chain.
var SNAPSHOT_FILES = Object.freeze([
  "db.key.enc",
  "hermitstash.db.enc",
  "vault.key",
  "vault.key.sealed",
].concat(TLS_FILES.map(function (t) { return t.local; })));
...
].concat(TLS_FILES.map(function (t) { return t.local + ".tmp"; })));
...
function createSnapshots(dataDir) {
  ...
  for (var i = 0; i < SNAPSHOT_FILES.length; i++) {
    ...
    var src = nodePath.join(dataDir, name);
    var snap = src + ".pre-restore";
    try {
      if (nodeFs.existsSync(src)) {
        nodeFs.copyFileSync(src, snap);
        created.push(name);
      }
    } catch (_e) {
...
function rollbackFromSnapshots(dataDir, snapshotsCreated, opts) {
  ...
  for (var r = 0; r < snapshotsCreated.length; r++) {
    ...
    var live = nodePath.join(dataDir, name);
    var snap = live + ".pre-restore";
    try {
      if (nodeFs.existsSync(snap)) {
        nodeFs.copyFileSync(snap, live);
      }
    } catch (e) {
  ...
  for (var t = 0; t < TMP_CLEANUP_FILES.length; t++) {
    try { nodeFs.unlinkSync(nodePath.join(dataDir, TMP_CLEANUP_FILES[t])); } catch (_e) { /* not always present */ }
  }
  if (opts.preserveSnapshots === false) {
    for (var c = 0; c < snapshotsCreated.length; c++) {
      try { nodeFs.unlinkSync(nodePath.join(dataDir, snapshotsCreated[c] + ".pre-restore")); } catch (_e) { /* best effort */ }
    }
  }
  return errors;
}
...
function clearSnapshots(dataDir, snapshotsCreated) {
  for (var i = 0; i < snapshotsCreated.length; i++) {
    try { nodeFs.unlinkSync(nodePath.join(dataDir, snapshotsCreated[i] + ".pre-restore")); } catch (_e) { /* may not exist */ }
  }
}
