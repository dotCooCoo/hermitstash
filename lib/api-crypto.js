...
function decryptPayload(sealed, keyBase64, replayWindow, maxBytes) {
  ...
  if (parsed && parsed._d !== undefined) {
    if (replayWindow) {
      // `_t` is attacker-chosen plaintext sealed inside the AEAD. Fail CLOSED when
      // the caller asked for replay enforcement: a missing or non-numeric `_t` would
      // make the skew arithmetic NaN, and every comparison against NaN is false, so
      // both the future- and expired-checks would silently pass — bypassing the
      // window entirely (the producer always stamps a numeric Date.now()). An
      // absolute-value window also symmetrically accepts a FUTURE-dated `_t`, which
      // slides the staleness check past the single-use nonce's TTL (anchored to
      // receive time) and opens a replay gap. Require a finite numeric timestamp,
      // then reject both a future one (beyond a small clock-skew tolerance) and an
      // expired one with one-sided checks.
