...
function decryptPayload(sealed, keyBase64, replayWindow) {
  ...
  // b.safeJson.parse's 1 MiB default cap would reject 1.5 MiB vault uploads.
  try { parsed = JSON.parse(plaintext); } catch (_e) { return null; }  // allow:bare-json-parse — AEAD-authenticated plaintext, size cap would block large vault ciphertext
