...
function saveChunk(bundleShareId, fileId, index, buffer) {
  ...
  if (!nodeFs.existsSync(dir)) nodeFs.mkdirSync(dir, { recursive: true });
  // Atomic, symlink-refusing sync write (CWE-59 / torn-write): temp + fsync + rename.
  b.atomicFile.writeSync(_chunkPath(bundleShareId, fileId, index), buffer, { fileMode: 0o600 });
}

function readChunk(bundleShareId, fileId, index) {
  // TOCTOU/symlink-safe read of a scratch chunk (CWE-367 / CWE-59).
  return b.atomicFile.fdSafeReadSync(_chunkPath(bundleShareId, fileId, index), {
    refuseSymlink: true, maxBytes: _LOCAL_READ_CAP,
  });
}
