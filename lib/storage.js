...
function getPresignedUrl(storagePath, filename, mimeType) {
  ...
  // Build the quoted Content-Disposition filename safely: strip control chars
  // (no header injection), then escape backslashes BEFORE quotes — escaping
  // quotes first would leave a lone backslash that closes the quoted-string
  // early, letting a stored filename break out of the value.
