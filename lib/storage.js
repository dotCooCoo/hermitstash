...
// Byte ceiling for buffered local reads (getRawBuffer + the buffered
// getFileStream branch). Stored blobs carry envelope/cipher overhead on top
// of the configured upload limit, so the cap is set well above maxFileSize —
// large enough never to reject a legitimate stored object, small enough to
// bound a hostile symlink-target read. Not a correctness limit; a DoS floor.
 ...
 * Every backend (local and S3) is app-encrypted; the returned encryptionKey is
 * the plaintext base64 key the caller stores on the file row and the DB layer
 * seals at rest. There is no unencrypted storage path — the PQC-only invariant
 * forbids relying on a classical S3 server-side cipher (AES) in its place.
 */
...
function resolveLocalPath(storagePath) {
  ...
  // storagePath (relative or absolute) against uploadDir and returns the confined
  // absolute path, or null on any escape — a `../` traversal, an absolute path
  // outside the root, or a sibling-prefix like `<uploadDir>-evil`. O_NOFOLLOW /
  // atomicFile guard the actual fs ops against a symlink swap.
...
// Per-upload chunk-encryption keys, held ONLY in process memory (keyed by
// "shareId/fileId"). Chunks are staged as XChaCha20-Poly1305 ciphertext so no
// plaintext fragment ever lands on the scratch disk — they persist across POSTs
// until reassembly and are GC'd after ~24h if abandoned. The key never touches
// disk: an abandoned upload's on-disk chunks are unreadable ciphertext, and a
// process restart mid-upload simply requires the client to re-upload (the
// orphaned ciphertext GCs). Node is single-threaded and saveChunk is sync, so the
// get-or-create is race-free across the separate per-chunk POST requests.
...
function statChunk(bundleShareId, fileId, index) {
  // Built OUTSIDE the try. _chunkPath refuses a component that would escape the
  // scratch directory and an index that is not a whole number; inside the try
  // those refusals were caught by a handler meant for "the chunk is not there",
  // so a caller that passed a malformed id was told the chunk was missing
  // instead of that its request was wrong — and the guard itself never ran.
...
// Total bytes of ALL chunks currently staged for a bundle, summed across every
// in-flight fileId assembly. This is the always-on per-bundle scratch backstop:
// it bounds accumulation regardless of the per-file / per-bundle POLICY caps
// (which an operator may set to 0 = "no limit"), so an attacker cannot stage
// many partial assemblies (distinct fileIds, last chunk never sent) to exhaust
// the scratch disk.
...
// Newest mtime (ms) of any staged chunk FILE anywhere under a bundle's scratch
// tree — the true "last write" signal for the bundle. The bundle directory's own
// mtime is not that signal: a directory's mtime advances only when an entry is
// created/removed IN it, so it reflects when the last fileId subdir was created,
// not when the last chunk was written into an existing assembly. Keying GC on the
// dir mtime therefore both (a) false-collects a bundle actively receiving chunks
// into an existing fileId (stale dir mtime) and (b) lets an attacker keep a bundle
// perpetually "fresh" by periodically creating a new fileId subdir. Walking to the
// newest chunk file fixes both. Falls back to the bundle dir's own mtime when it
// holds no chunk files yet (freshly-created, empty).
