 ...
 *
 * Both transports come from the framework, which carries the RFC 5321 §4.5.3.1
 * line caps, RFC 3030 BDAT chunking honouring the peer's advertised size, the
 * RFC 1870 SIZE precheck, EAI and SMTPUTF8 (RFC 6531) with IDN Punycode
 * (RFC 3492), and the shared outbound TLS posture — TLS 1.3 floor,
 * hybrid group order and certificate compression — all worth more
 * than the mostly-duplicated bytes the hand-rolled state machine
 * produced.
 */
...
function buildMailers() {
  smtpMailer = config.email.host ? b.mail.create({
    transport: b.mail.transports.smtp({
      ...
      // No ecdhCurve here: the transport merges b.network.tls.outboundPosture()
      // and a caller option overrides it wholesale. Passing the framework's own
      // group string back in changed nothing while making the dial stop
      // following the shared posture the moment that posture moved.
...
function getQuotaCounts() {
  ...
  //
  // Only Resend-backend rows count: the cap (resendQuotaDaily/Monthly) is
  // Resend-specific, so in a smtp+resend combo an SMTP send (backend "smtp") must
  // not consume — and thus can't exhaust — the Resend fallback quota. backend is
  // set on every row by trackSend and auto-unsealed on read.
  function counted(s) { return (s.status === "sent" || s.status === "pending") && s.backend === "resend"; }
...
function tpl(str, vars) {
  // Resolve a {name} placeholder as an OWN property only. \w+ matches inherited
  // Object.prototype member names (constructor, __proto__, toString), and a bare
  // `vars[key] !== undefined` treats those as present — splicing a function's
  // source (or a prototype-polluted value) into the rendered email. An unknown or
  // inherited name is left as the literal {name}, matching the own-property
  // discipline the framework's own template/message-format renderers use.
  return str.replace(/\{(\w+)\}/g, function (_, key) {
    return Object.prototype.hasOwnProperty.call(vars, key) && vars[key] !== undefined ? vars[key] : "{" + key + "}";
  });
}
...
// The href is the one value in these templates that is computed rather than
// written literally, and it lands inside a quoted attribute. Today every caller
// passes getOrigin() plus a server-generated token, and getOrigin reads operator
// configuration rather than the request's Host header — so nothing an attacker
// controls reaches here. That is a property of the callers, not of this
// function, and it is the kind that changes quietly: one caller built from a
// slug or a display name and a quote in the value would close the attribute and
// let the rest of it into the message.
//
// Escaping costs nothing here. The escaped value decodes back to the original
// URL, so the link is unchanged for the reader, and a quote can no longer end
// the attribute. b.forms.escapeAttribute rather than the HTML-text escaper: it
// is the attribute-context primitive, it also covers the backtick and `=` that
// an unquoted-attribute slip would expose, and it renders a nullish value as the
// empty string instead of the word "undefined".
...
<a href="${b.forms.escapeAttribute(href)}" style="color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;display:inline-block">${text}</a>
</td></tr></table>`;
}
...
// _emailButton joins _renderTpl as a test seam. The composed message is not
// otherwise observable — the mailers are module-level and built from config — so
// without it the escaping of the one computed value in these templates could
// only be asserted by sending mail.
module.exports = { sendEmail, sendUploaderConfirmation, sendAdminNotification, sendVerificationEmail, sendInviteEmail, sendPasswordResetEmail, sendBundleAccessCode, checkQuota, getQuotaCounts, _renderTpl: renderTpl, _emailButton: emailButton };
