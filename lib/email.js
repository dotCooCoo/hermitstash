...
function smtpSend({ to, subject, html }) {
  ...
  return new Promise((resolve) => {
    ...
    function connect() {
      if (useImplicitTLS) {
        socket = tls.connect(port, config.email.host, { rejectUnauthorized: config.smtpRejectUnauthorized !== false, ecdhCurve: b.constants.TLS_GROUP_CURVE_STR, minVersion: "TLSv1.3" }, () => {});
      } else {
    ...
    function handleResponse(code) {
      if (step === 0) { send("EHLO hermitstash"); step = 1; }
        ...
        var tlsSocket = tls.connect({ socket, rejectUnauthorized: config.smtpRejectUnauthorized !== false, servername: config.email.host, ecdhCurve: b.constants.TLS_GROUP_CURVE_STR, minVersion: "TLSv1.3" }, () => {
