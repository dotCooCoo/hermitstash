  ...
  return new Promise((resolve) => {
    ...
    function handleResponse(code) {
      if (step === 0) { send("EHLO hermitstash"); step = 1; }
        ...
        const tlsSocket = tls.connect({ socket, rejectUnauthorized: config.smtpRejectUnauthorized !== false, servername: config.email.host, ecdhCurve: TLS_GROUP_CURVE_STR, minVersion: "TLSv1.3" }, () => {
