...
function smtpSend({ to, subject, html }) {
  ...
  return new Promise((resolve) => {
    ...
    function connect() {
      if (useImplicitTLS) {
        socket = tls.connect(port, config.email.host, { rejectUnauthorized: config.smtpRejectUnauthorized !== false, ecdhCurve: "X25519MLKEM768:SecP256r1MLKEM768", minVersion: "TLSv1.3" }, () => {});
      } else {
    ...
    function handleResponse(code) {
      if (step === 0) { send("EHLO hermitstash"); step = 1; }
        ...
        const tlsSocket = tls.connect({ socket, rejectUnauthorized: config.smtpRejectUnauthorized !== false, servername: config.email.host, ecdhCurve: "X25519MLKEM768:SecP256r1MLKEM768", minVersion: "TLSv1.3" }, () => {
