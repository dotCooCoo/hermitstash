...
function smtpSend({ to, subject, html }) {
  ...
  return new Promise((resolve) => {
    ...
    function connect() {
      if (useImplicitTLS) {
        socket = tls.connect(port, config.email.host, { rejectUnauthorized: config.smtpRejectUnauthorized !== false }, () => {});
      } else {
    ...
    function handleResponse(code) {
      if (step === 0) { send("EHLO hermitstash"); step = 1; }
        ...
        const tlsSocket = tls.connect({ socket, rejectUnauthorized: config.smtpRejectUnauthorized !== false, servername: config.email.host }, () => {
...
module.exports = { sendEmail, sendUploaderConfirmation, sendAdminNotification, sendVerificationEmail, sendInviteEmail, sendPasswordResetEmail, checkQuota, getQuotaCounts };
