// codebase-patterns:allow-file raw-byte-literal — SMTP step numbers, HTML template literal byte sizes (font-size, padding), inline HTML pixel values
/**
...
function resendSend({ to, subject, html }) {
  ...
  return b.httpClient.request({
    ...
    try {
      var data = b.safeJson.parse(text);
      if (data && data.id) return true;
      logger.error("Resend error", { error: (data && data.message) || text });
      return false;
    } catch (_e) {
    ...
    logger.error("Resend error", { error: err.message || String(err) });
    return false;
  });
}
...
function smtpSend({ to, subject, html }) {
  ...
  return new Promise((resolve) => {
    ...
    function connect() {
      if (useImplicitTLS) {
        socket = nodeTls.connect(port, config.email.host, { rejectUnauthorized: config.smtpRejectUnauthorized !== false, ecdhCurve: b.constants.TLS_GROUP_CURVE_STR, minVersion: "TLSv1.3" }, () => {});
      } else {
        socket = net.createConnection(port, config.email.host, () => {});
      }
      socket.setEncoding("utf8");
      socket.setTimeout(C.TIME.seconds(15));
    ...
    function handleResponse(code) {
      if (step === 0) { send("EHLO hermitstash"); step = 1; }
        ...
        var tlsSocket = nodeTls.connect({ socket, rejectUnauthorized: config.smtpRejectUnauthorized !== false, servername: config.email.host, ecdhCurve: b.constants.TLS_GROUP_CURVE_STR, minVersion: "TLSv1.3" }, () => {
