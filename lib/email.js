...
const { formatSize } = require("./template");
...
async function trySend(backend, to, subject, html) {
  if (backend === "resend") {
    ...
    if (!quota.allowed) {
      ...
      try { audit().log(audit().ACTIONS.EMAIL_QUOTA_EXCEEDED, { details: quota.reason + ", to: " + to }); } catch (_e) {}
  ...
  for (var i = 0; i < order.length; i++) {
    ...
    try {
      ...
      if (result.success) {
        ...
        try { audit().log(audit().ACTIONS.EMAIL_SENT, { details: "backend: " + backend + (isFallback ? " (fallback)" : "") + ", to: " + to }); } catch (_e) {}
        return true;
      }
      trackSend(to, subject, backend, "failed");
      try { audit().log(audit().ACTIONS.EMAIL_SEND_FAILED, { details: "backend: " + backend + ", reason: " + (result.reason || "send failed") + ", to: " + to }); } catch (_e) {}
    } catch (e) {
      trackSend(to, subject, backend, "failed");
      try { audit().log(audit().ACTIONS.EMAIL_SEND_FAILED, { details: "backend: " + backend + ", error: " + e.message + ", to: " + to }); } catch (_e2) {}
    }
  }
...
function smtpSend({ to, subject, html }) {
  ...
  return new Promise((resolve) => {
    ...
    function connect() {
      if (useImplicitTLS) {
        socket = tls.connect(port, config.email.host, { rejectUnauthorized: config.smtpRejectUnauthorized !== false, ecdhCurve: "X25519MLKEM768:SecP256r1MLKEM768:X25519:prime256v1", minVersion: "TLSv1.3" }, () => {});
      } else {
    ...
    const fromAddr = extractEmail(config.email.from);
    const message = [
      `From: ${config.email.from}`,
      `To: ${to}`,
      `Subject: ${subject}`,
    ...
    function handleResponse(code) {
      if (step === 0) { send("EHLO hermitstash"); step = 1; }
        ...
        const tlsSocket = tls.connect({ socket, rejectUnauthorized: config.smtpRejectUnauthorized !== false, servername: config.email.host, ecdhCurve: "X25519MLKEM768:SecP256r1MLKEM768:X25519:prime256v1", minVersion: "TLSv1.3" }, () => {
