...
function getDbEncKey() {
  ...
  // Store a vault-sealed key for DB file encryption.
  if (nodeFs.existsSync(_dbKeyPath)) {
    try {
      ...
      // Unsealing failed. This is INDISTINGUISHABLE from two very different
      // situations: (a) the sealed key file is genuinely corrupt, or (b) the
      // vault keypair on disk no longer matches the one db.key.enc was sealed
      // to — a Poly1305 tag failure from a lost / mismatched data/vault.key
      // (e.g. restored from the wrong backup). Both throw here.
      //
      // FAIL CLOSED when db.enc already exists. Regenerating the DB key and
      // re-sealing db.key.enc would overwrite the ONLY sealed copy of the real
      // key with one sealed to the wrong vault keypair — making the existing
      // db.enc permanently undecryptable even after the correct vault.key is
      // restored (IRREVERSIBLE data loss). Refuse to start instead; the
      // operator restores the matching vault.key and reboots cleanly.
      ...
      // No db.enc on disk: there is no encrypted database to lose, so a
      // corrupt / unreadable db.key.enc can safely be regenerated.
  ...
  // Verify-before-reseal: if a db.enc already exists, the new key MUST be able
  // to decrypt it before we overwrite db.key.enc — otherwise we'd strand the
  // database. A genuine first run reaches here with no db.enc (db.key.enc was
  // absent above, or absent-and-corrupt with no db.enc), so the fresh key is
  // correct by construction. Any path where db.enc exists but the new key
  // can't read it is a contradiction (the unseal-failure branch already
  // exits when db.enc exists) — fail closed rather than clobber db.key.enc.
  if (encPath && nodeFs.existsSync(encPath)) {
  ...
  // Atomic write (temp + fsync + rename-with-retry): a torn write of the sealed
  // DB key would, on the next boot, fail to unseal → without the fail-closed
  // guard above a NEW key would be minted → the encrypted DB becomes permanently
  // unreadable. Crash-atomic replacement closes that data-loss path (and rides
  // out Windows/Dropbox locks).
...
function decryptDbFile() {
  ...
  var packed = nodeFs.readFileSync(encPath);
  if (packed.length < 26) {
    // Too short to be an envelope. Returning here used to be silent, and the
    // silence was the damage: with the plaintext database on tmpfs there is
    // nothing else to load, so the server came up on an empty database and the
    // next re-encrypt — periodic or at exit — wrote that empty database over
    // the only durable copy. The comment on encryptDbFile below names this
    // outcome exactly, which is why it writes atomically; that stops this
    // process tearing the file, not a file torn by a full disk, an interrupted
    // copy or a partial restore.
    //
    // getDbEncKey treats the identical length test as fatal thirty lines above.
    // Two responses to one signal, and this was the one that discards data.
  ...
  // The codec tries the dataDir-bound AAD first, then falls back to an un-bound
  // decrypt — a rotated db.enc is AAD-bound (b.vaultRotate.rotate), an existing
  // pre-AAD file or a portable backup is not. Without this the server was
  // unbootable after any vault-key rotation.
  b.atomicFile.writeSync(dbPath, dbEncCodec.decryptDbEnc(packed, getDbEncKey(), dataDir));
}

// Set once a restore has written an authoritative db.enc to disk: the exit-time and
// periodic re-encrypt must then NOT run — they would overwrite the restored bytes
// with the stale in-memory tmpfs DB, re-encrypted under the OLD key (reverting the
// restore, or bricking the next boot when db.key.enc also changed).
...
function encryptDbFile(removePlaintext) {
  ...
  // Atomic write: a torn .db.enc (truncated past the 26-byte guard) fails to
  // decrypt on boot → the key catch regenerates → permanent data loss. This runs
  // every 5 min, on exit, and during backup snapshots, so durability matters.
  // AAD-bind the live db.enc to the data directory (matches what
  // b.vaultRotate.rotate writes and blamejs's own db.js), so another install
  // sharing the operator passphrase can't swap its db.enc in. The boot read above
  // falls back to an un-bound decrypt for the one-time transition from an existing
  // no-AAD file.
...
// Produce an encrypted snapshot of the CURRENT DB state as a Buffer,
// independent of encPath. Used by backup to capture up-to-date DB bytes
// even when:
//   - encPath is null (HERMITSTASH_DB_PATH override / test mode), OR
//   - the periodic 5-minute encrypt hasn't fired since the last write.
//
// Checkpoints WAL first so the .db file contains all committed transactions.
// Returns a packed Buffer matching encPath's format — callers can write it
// wherever they want without disturbing the live encPath file.
function snapshotEncryptedDb() {
  ...
  // Backup snapshots are intentionally NOT AAD-bound to the data directory: a
  // backup must restore across installs / machines whose DATA_DIR differs (disaster
  // recovery), which a dataDir-bound AAD would break. The boot read (decryptDbFile)
  // falls back to an un-bound decrypt, so a restored no-AAD snapshot still opens;
  // the live db.enc keeps its AAD swap-protection independently.
...
// limits.sqlLength rejects a statement over 1 MiB at parse time — a DoS floor on
// the raw-SQL surface, complementary to the row-count gate. (node:sqlite limits
// land in Node 24.10+, under the 24.18 floor.) Mirrors the vendored blamejs handle.
...
// Security pragmas (mirroring the vendored blamejs DB handle):
// secure_delete=ON overwrites freed pages with zeros so deleted SEALED values
//   can't be carved from the DB file (crypto-shred for erased rows);
// trusted_schema=OFF refuses functions / virtual-table modules referenced from a
//   planted shadow schema (CVE-2018-8740 family — relevant for backup/restore);
// cell_size_check=ON rejects corrupted pages at parse time instead of crashing.
...
// Called by the restore endpoint after the restore worker writes a fresh,
// authoritative db.enc / db.key.enc to disk and before it schedules the restart:
// stop the exit-time + periodic re-encrypt so the graceful shutdown can't clobber
// the restored bytes with the stale in-memory tmpfs DB.
...
try { db.exec("ALTER TABLE users ADD COLUMN pendingEmail TEXT"); } catch (_e) { /* already exists */ }
// UNIQUE so the database itself rejects a second account with the same email,
// closing the registration TOCTOU (the sync findByEmail → await argon2.hash →
// insert window). customer_stash.slugHash uses the same pattern. SQLite treats
// NULLs as distinct under UNIQUE, so email-less (OAuth) accounts are unaffected.
// On an upgrade that already holds duplicate emailHashes the UNIQUE create
// throws → fall back to the non-unique index (the app-level existing-email check
// stays the guard) and leave the operator to dedup.
try {
...
try { db.exec("ALTER TABLE users ADD COLUMN totpAlgorithm TEXT"); } catch (_e) { /* already exists */ }
...
// Per-user upload-limit overrides. 0 / null = "not set" → fall back to the global
// config, so these are off by default. Numeric limits stay raw (used in arithmetic,
// like customer_stash's); allowedExtensions is sealed (a string, like the stash's).
try { db.exec("ALTER TABLE users ADD COLUMN quotaBytes INTEGER DEFAULT 0"); } catch (_e) { /* already exists */ }
try { db.exec("ALTER TABLE users ADD COLUMN maxFileSize INTEGER DEFAULT 0"); } catch (_e) { /* already exists */ }
try { db.exec("ALTER TABLE users ADD COLUMN maxFiles INTEGER DEFAULT 0"); } catch (_e) { /* already exists */ }
try { db.exec("ALTER TABLE users ADD COLUMN maxBundleSize INTEGER DEFAULT 0"); } catch (_e) { /* already exists */ }
try { db.exec("ALTER TABLE users ADD COLUMN allowedExtensions TEXT"); } catch (_e) { /* already exists */ }
// Tailscale SSO: tailscaleId is the sealed stable tailnet login identity;
// tailscaleIdHash is its keyed-MAC blind index for the indexed lookup (a
// distinct identity namespace from email — a tailnet login that coincides with
// an existing email never cross-matches). Non-UNIQUE: a fresh install adds the
// column empty-for-all-rows, and an accidental duplicate must not brick the
// whole users table at boot the way a UNIQUE create would.
try { db.exec("ALTER TABLE users ADD COLUMN tailscaleId TEXT"); } catch (_e) { /* already exists */ }
try { db.exec("ALTER TABLE users ADD COLUMN tailscaleIdHash TEXT"); } catch (_e) { /* already exists */ }
...
// Richer request context (who/what/when/where/how). method / authType / requestId
// stay RAW (enum-ish verb + auth class + correlation id); path + userAgent are
// SEALED (they carry identifiers). lib/audit.js captures these from the request
// when one is present; system events leave them NULL.
...
try { db.exec("ALTER TABLE customer_stash ADD COLUMN teamId TEXT"); } catch (_e) { /* already exists */ }
...
// Idempotency-Key cache: HS used to ship its own `idempotency_keys` table
// with field-crypto sealing; v1.10.7+ delegates to b.middleware.idempotencyKey.dbStore
// which creates its own `blamejs_idempotency_keys` table at first use
// (init=true, hashKeys=true, seal=true). Drop the legacy HS-managed
// table on upgrade — the cache is transient (24h TTL) so no operator
// data is lost.
...
// Register HS's tables with b.cryptoField up front: sealDoc/unsealDoc delegate
// to b.cryptoField.sealRow/unsealRow, which silently no-op (store PLAINTEXT) for
// an unregistered table. Registration is idempotent + key-independent, so it is
// safe at module load (the rotation/verify scripts also register explicitly).
var fieldCrypto = require("./field-crypto");
fieldCrypto.registerWithBlamejs();

// Build one SQL WHERE condition + its bound values for a query field value.
// Supports plain equality, { $ne }, and { $in: [...] }. The $in form backs the
// derived-hash dual-read (field-crypto.lookupHash) so a blind-index lookup
// matches both the active keyed-MAC digest and the legacy unkeyed digest while
// the keyed-index backfill (lib/derived-hash-backfill) is in progress.
...
class Collection {
  ...
  _merge(row) {
    ...
    if (row.data) {
      try {
        ...
        // A real column ALWAYS wins over the overflow blob. On an upgraded DB the
        // blob may still carry a key that has since been promoted to a real column;
        // a blind Object.assign(doc, blob) would clobber the current column value
        // with the stale blob one. Only merge keys that are not real columns, and
        // never a reserved prototype key (defense-in-depth alongside safeJson).
        Object.keys(blob).forEach(function (k) {
          if (k === "__proto__" || k === "constructor" || k === "prototype") return;
          // Computed write via b.safeObject.ownSet (prototype-pollution-safe own-set)
          // rather than a raw doc[k]=, consolidating onto the framework primitive; the
          // explicit reserved-key skip above stays as belt-and-suspenders.
          if (realCols.indexOf(k) === -1) b.safeObject.ownSet(doc, k, blob[k]);
        });
      } catch { /* malformed overflow blob — real columns still apply */ }
    }
    // Unseal FIRST: a column that is BOTH sealed and JSON-typed (e.g.
    // users.totpBackupCodes) is still ciphertext at this point, so parsing it as
    // JSON below would throw and leave it a string — decrypt it to its plaintext
    // JSON before the JSON-column parse runs.
  ...
  // Is this column stored as ciphertext? The question the query guard has to
  // ask — "does it carry a derived index" is a different and much narrower one,
  // true for only a handful of fields, and using it left every other sealed
  // column answering queries wrongly in silence.
  //
  // Cached per collection: getSealedFields walks the field-crypto tables, and
  // this runs for every key of every query.
  _isSealedColumn(key) {
    ...
    for (var key in query) {
      ...
      if (val && typeof val === "object" && Array.isArray(val.$in)) {
        ...
        if (derivedKey && val.$in.length > 0) {
          ...
          // An empty list is answerable without touching the column at all:
          // _sqlCond renders it as `0 = 1`. Callers build these lists
          // dynamically, so "none of them" is an ordinary query, not one the
          // storage cannot evaluate — refusing it would break a working shape
          // in the name of a guard against wrong answers.
          translated[key] = val;
        } else if (this._isSealedColumn(key)) {
          // Sealed, but the blind index could not be used — an empty list, a
          // non-string element, or a field with no derived index. Passing it
          // through compares the operand against ciphertext and quietly matches
          // nothing. Refuse instead: a caller that asked a question the storage
          // cannot answer needs to hear so, not receive an empty result.
      ...
      if (lookup && typeof val === "string") {
        ...
        // An absent or blank operand. Writes create no digest for an empty
        // source, so nothing can match it, and an empty result is exactly what
        // this returned before the guard existed — by comparing the blank
        // against the ciphertext column and finding nothing. Keep the answer and
        // drop the mechanism: an empty $in renders as `0 = 1`, identical result,
        // no predicate that only happened to be right.
        //
        // `undefined` belongs here with the rest. Treating it as a caller error
        // and raising reads well in the abstract, and it broke the certificate
        // reissue path in the end-to-end suite — a live caller reaches this with
        // no value and has always been answered "no match". The defect being
        // fixed is a query returning the WRONG rows; turning queries that
        // already returned the right answer into exceptions is a different and
        // unasked-for change.
        translated[key] = { $in: [] };
      } else if (this._isSealedColumn(key)) {
        // Sealed, and it did not blind-index above. It cannot be evaluated
        // against the ciphertext column: passing it through compiles a silently
        // wrong predicate — `sealedCol IS NOT ?` matches every row, `sealedCol =
        // ?` matches none — so a query written to EXCLUDE something returns it.
        //
        // This used to be gated on the field carrying a derived index, which
        // meant it only fired for the handful of fields that have one. A merely
        // sealed column — authType, totpEnabled, originalName — fell to the
        // pass-through this comment warns about and answered wrongly without a
        // word. searchPaginated has always tested the right thing; now so does
        // this. Only equality and $in over a derived index are answerable.
        // Boolean-ish sealed columns are refused too, deliberately. The write
        // path stores `false` as a raw "0.0" and `true` as ciphertext in the
        // same column, so `{vaultEnabled: false}` used to match by accident of
        // that encoding while `{vaultEnabled: true}` matched nothing. Allowing
        // the falsy half back would keep one value of a boolean queryable and
        // the other silently not — the shape this guard exists to end. No
        // caller queries one; if a future one needs to, the fix is to store the
        // flag raw, not to compare against ciphertext.
  ...
  update(query, ops) {
    // Sealed columns are AAD-bound to their row's _id (b.cryptoField), so they
    // must be sealed PER ROW with that row's _id — incompatible with the
    // multi-row single-UPDATE fast path. We no longer pre-seal $set up front
    // (the old single seal had no _id); sealing moves into the per-row slow path.
    ...
    // Fast path: single UPDATE — only when $set is raw-only (no sealed column).
    // Raw columns (counters, status enums, FK ids, timestamps) need no sealing,
    // so a multi-row single-statement UPDATE is safe and stays the hot path.
    ...
    // Slow path: per-row. Load raw (still-sealed) rows, apply the plaintext $set,
    // then seal EACH row with its own _id so AAD binds correctly. Untouched
    // sealed columns are read back already-sealed and pass through verbatim (the
    // sealDoc skip-guard) — lazy migration: only changed columns are re-sealed.
    ...
    for (var i = 0; i < docs.length; i++) {
      ...
      if (ops.$push) {
        ...
        // The slow-path rows above are loaded RAW (sealed columns are ciphertext,
        // JSON columns unparsed) so $set's untouched columns pass through verbatim.
        // $push needs the DECODED array, so re-read this row through the sealed
        // reader (plaintext arrays), append, then reseal any sealed column touched —
        // otherwise the push would drop existing entries AND store the array as
        // plaintext (a sealed column silently de-sealed).
