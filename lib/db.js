...
const encPath = process.env.HERMITSTASH_DB_PATH ? null : path.join(dataDir, "hermitstash.db.enc");

// Derive AES-256 DB encryption key via ML-KEM-768.
...
var _dbKey = null;
var _dbKeyPath = path.join(dataDir, "db.key.enc");
...
function getDbEncKey() {
  ...
  // Generate new DB encryption key and seal with ML-KEM-768
  var vault = require("./vault");
...
process.on("SIGINT", function () { try { encryptDbFile(true); } catch (_e) {} process.exit(0); });
process.on("SIGTERM", function () { try { encryptDbFile(true); } catch (_e) {} process.exit(0); });
...
// Vault file columns (ML-KEM-768 client-side encrypted uploads)
...
const COLUMNS = {
  ...
  audit_log: ["_id", "action", "targetId", "targetEmail", "performedBy", "performedByEmail", "details", "createdAt"],
  files: ["_id", "shareId", "shareIdHash", "bundleId", "bundleShareId", "bundleShareIdHash", "uploadedBy", "uploaderEmail", "originalName", "relativePath", "storagePath", "mimeType", "size", "downloads", "status", "encryptionKey", "expiresAt", "emailHash", "teamId", "vaultEncrypted", "vaultEncapsulatedKey", "vaultIv", "vaultBatchId", "vaultBatchName", "checksum", "createdAt"],
  bundles: ["_id", "shareId", "shareIdHash", "uploaderName", "uploaderEmail", "expectedFiles", "receivedFiles", "skippedCount", "totalSize", "downloads", "status", "emailHash", "passwordHash", "message", "expiresAt", "finalizeTokenHash", "skippedFiles", "ownerId", "teamId", "stashId", "bundleName", "allowedEmails", "accessMode", "createdAt"],
  blocked_ips: ["_id", "ip", "reason", "blockedBy", "createdAt"],
  api_keys: ["_id", "name", "keyHash", "prefix", "permissions", "userId", "lastUsed", "createdAt"],
  ...
  customer_stash: ["_id", "slug", "slugHash", "name", "title", "subtitle", "accentColor", "logoUrl", "passwordHash", "maxFileSize", "maxFiles", "maxBundleSize", "defaultExpiry", "allowedExtensions", "allowedEmails", "accessMode", "enabled", "createdBy", "bundleCount", "totalBytes", "createdAt"],
...
(function migrateOverflowFields() {
  var migrations = [
    { table: "users", fields: ["totpSecret", "totpEnabled", "totpBackupCodes"] },
    { table: "files", fields: ["checksum"] },
    { table: "bundles", fields: ["finalizeTokenHash", "skippedFiles"] },
  ];
