    ...
    for (var key in query) {
      ...
      if (val && typeof val === "object" && Array.isArray(val.$in)) {
        ...
        if (derivedKey && val.$in.length > 0) {
          ...
          translated[key] = val; // raw column or non-string element — pass through
        }
        continue;
      }
      ...
      if (lookup && typeof val === "string") {
        ...
        translated[lookup.key] = (lookup.candidates && lookup.candidates.length > 1)
          ...
          : lookup.value;
                 ...
                 fieldCrypto.lookupHash(this.name, key, "probe")) {
        // A sealed/derived field carries an OPERATOR query ($ne, $gt, …) that is
        // not the handled equality / $in case. It can't be evaluated against the
        // ciphertext column — passing it through compiles a silently-wrong
        // predicate (e.g. `sealedCol IS NOT ?` is always true, matching nothing /
        // everything). Fail loud so a caller gets an error rather than a wrong
        // result; only equality and $in are blind-indexed. (Raw columns fall to
        // the pass-through below.)
        throw new Error("Unsupported query operator on sealed/derived field '" + key +
          "' in " + this.name + " — only equality and $in are blind-indexed");
      } else {
        translated[key] = val;
      }
    }
