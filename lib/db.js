...
var dbBaseName = "hermitstash-" + b.crypto.generateToken(32) + ".db";
...
function getDbEncKey() {
  ...
  var vault = require("./vault");
  var rawKey = b.crypto.generateBytes(32);
...
class Collection {
  ...
  _genId() {
    return b.crypto.generateToken(32);
  }
  ...
  _merge(row) {
    ...
    if (row.data) {
      try { Object.assign(doc, JSON.parse(row.data)); } catch { /* malformed overflow blob — real columns still apply */ }
    }
    ...
    for (var j = 0; j < this.jsonColumns.length; j++) {
      ...
      if (typeof doc[jc] === "string") {
        try { doc[jc] = JSON.parse(doc[jc]); } catch (_e) { /* leave as string if not valid JSON */ }
      }
    }
