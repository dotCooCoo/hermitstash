...
function getDbEncKey() {
  ...
  nodeFs.writeFileSync(_dbKeyPath, sealed, { mode: 0o600 });
  _dbKey = rawKey;
  return _dbKey;
}
...
function decryptDbFile() {
  ...
  nodeFs.writeFileSync(dbPath, b.crypto.decryptPacked(packed, getDbEncKey()));
}
...
function encryptDbFile(removePlaintext) {
  ...
  nodeFs.writeFileSync(encPath, b.crypto.encryptPacked(nodeFs.readFileSync(dbPath), getDbEncKey()));
  if (removePlaintext) {
