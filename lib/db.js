...
function getDbEncKey() {
  ...
  // DB key would, on the next boot, fail to unseal → the catch above regenerates
  // a NEW key → the encrypted DB becomes permanently unreadable. Crash-atomic
  // replacement closes that data-loss path (and rides out Windows/Dropbox locks).
