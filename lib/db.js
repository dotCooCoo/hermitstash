...
function decryptDbFile() {
  ...
  b.atomicFile.writeSync(dbPath, b.crypto.decryptPacked(packed, getDbEncKey()));
}
...
function encryptDbFile(removePlaintext) {
  ...
  b.atomicFile.writeSync(encPath, b.crypto.encryptPacked(nodeFs.readFileSync(dbPath), getDbEncKey()), { fileMode: 0o600 });
  if (removePlaintext) {
...
class Collection {
  ...
  _merge(row) {
    ...
    if (row.data) {
      try { Object.assign(doc, JSON.parse(row.data)); } catch { /* malformed overflow blob — real columns still apply */ } // allow:bare-json-parse — parsing overflow JSON we wrote ourselves in this DB module
    }
