...
// land in Node 24.10+, under the 24.16 floor.) Mirrors the vendored blamejs handle.
  ...
  for (var m = 0; m < migrations.length; m++) {
    ...
    try {
      ...
      for (var r = 0; r < rows.length; r++) {
        var extra;
        try { extra = JSON.parse(rows[r].data); } catch (_e) { continue; } // allow:bare-json-parse — parsing overflow JSON we wrote ourselves in this DB module
...
class Collection {
  ...
  _merge(row) {
    ...
    if (row.data) {
      try {
        var blob = JSON.parse(row.data); // allow:bare-json-parse — parsing overflow JSON we wrote ourselves in this DB module
        ...
        // with the stale blob one. Only merge keys that are not real columns.
