...
try { db.exec("ALTER TABLE api_keys ADD COLUMN boundStashId TEXT"); } catch (_e) { /* already exists */ }
try { db.exec("ALTER TABLE api_keys ADD COLUMN boundBundleId TEXT"); } catch (_e) { /* already exists */ }
...
const COLUMNS = {
  ...
  api_keys: ["_id", "name", "keyHash", "prefix", "permissions", "userId", "lastUsed", "createdAt", "boundStashId", "boundBundleId"],
  ...
  enrollment_codes: ["_id", "codeHash", "apiKey", "clientCert", "clientKey", "caCert", "stashId", "bundleId", "createdBy", "status", "expiresAt", "createdAt"],
};
