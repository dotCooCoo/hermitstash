...
function encryptDbFile(removePlaintext) {
  ...
  if (removePlaintext) {
    ...
    try { nodeFs.unlinkSync(dbPath + ".owner"); } catch (_e) { /* cleanup — owner marker may not exist */ }
  }
}
...
// Record this process as the owner of its plaintext database, so the sweep
// below can tell a file a crash left behind from one a live process is still
// writing to. dbPath carries a fresh random name every start, so without this
// every process treats every OTHER process's database as stale.
var ownerPath = dbPath + ".owner";
try { nodeFs.writeFileSync(ownerPath, String(process.pid), { mode: 0o600 }); } catch (_e) { /* tmpDir may not exist yet on first run */ }

// Signal 0 tests for the process without delivering anything. ESRCH means it is
// gone; EPERM means it exists under another uid, which still counts as alive. A
// file with no owner recorded is from a build that predates this or from a crash
// before the write, and is swept as before. A reused pid makes this answer
// "alive" for a file that is actually stale, which leaks a file rather than
// destroying a live one.
function _ownerAlive(dbFilePath) {
  var pid;
  try { pid = parseInt(nodeFs.readFileSync(dbFilePath + ".owner", "utf8").trim(), 10); }
...
// Plaintext databases left behind by a crashed process. Unlinking one a live
// process still holds does not stop that process: it keeps serving from the open
// fd, while every later encryptDbFile() returns at its existsSync check without
// writing and without logging, so nothing it writes from that point on reaches
// db.enc.
try {
  var staleFiles = nodeFs.readdirSync(tmpDir).filter(function (f) {
    if (!f.startsWith("hermitstash-") || !f.endsWith(".db")) return false;
    var full = nodePath.join(tmpDir, f);
    if (full === dbPath) return false;
    return !_ownerAlive(full);
  });
  staleFiles.forEach(function (f) {
    ...
    try { nodeFs.unlinkSync(nodePath.join(tmpDir, f + ".owner")); } catch (_e) { /* cleanup — owner marker may not exist */ }
  });
...
//
// The anchor is signed. It is the one row that licenses missing rows, so the
// framework refuses a chain whose anchor carries no signature. The columns
// below hold that signature and its bindings, and a mirror missing any of
// them makes every prune fail.
db.exec(`
...
`);
// Databases created before the anchor was signed carry the five original
// columns only.
try { db.exec("ALTER TABLE _blamejs_audit_purge_anchor ADD COLUMN firstPurgedCounter INTEGER NOT NULL DEFAULT 0"); } catch (_e) { /* column may already exist */ }
try { db.exec("ALTER TABLE _blamejs_audit_purge_anchor ADD COLUMN archiveRowsDigest TEXT"); } catch (_e) { /* column may already exist */ }
try { db.exec("ALTER TABLE _blamejs_audit_purge_anchor ADD COLUMN archiveCheckpointDigest TEXT"); } catch (_e) { /* column may already exist */ }
try { db.exec("ALTER TABLE _blamejs_audit_purge_anchor ADD COLUMN archiveManifestDigest TEXT"); } catch (_e) { /* column may already exist */ }
try { db.exec("ALTER TABLE _blamejs_audit_purge_anchor ADD COLUMN signature BLOB"); } catch (_e) { /* column may already exist */ }
try { db.exec("ALTER TABLE _blamejs_audit_purge_anchor ADD COLUMN publicKeyFingerprint TEXT"); } catch (_e) { /* column may already exist */ }
try { db.exec("ALTER TABLE _blamejs_audit_purge_anchor ADD COLUMN fencingToken INTEGER NOT NULL DEFAULT 0"); } catch (_e) { /* column may already exist */ }
