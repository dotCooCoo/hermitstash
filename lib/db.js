...
var dbPath = process.env.HERMITSTASH_DB_PATH || path.join(tmpDir, dbBaseName);
...
function decryptDbFile() {
  ...
  fs.writeFileSync(dbPath, b.crypto.decryptPacked(packed, getDbEncKey()));
}
...
function encryptDbFile(removePlaintext) {
  ...
  fs.writeFileSync(encPath, b.crypto.encryptPacked(fs.readFileSync(dbPath), getDbEncKey()));
  if (removePlaintext) {
    try { fs.unlinkSync(dbPath); } catch (_e) { /* cleanup — file may already be gone */ }
    try { fs.unlinkSync(dbPath + "-wal"); } catch (_e) { /* cleanup — WAL file may not exist */ }
    try { fs.unlinkSync(dbPath + "-shm"); } catch (_e) { /* cleanup — SHM file may not exist */ }
  }
}
...
try {
  ...
  staleFiles.forEach(function (f) {
    try { fs.unlinkSync(path.join(tmpDir, f)); } catch (_e) { /* cleanup — stale DB file may have been removed concurrently */ }
    try { fs.unlinkSync(path.join(tmpDir, f + "-wal")); } catch (_e) { /* cleanup — stale WAL may not exist */ }
    try { fs.unlinkSync(path.join(tmpDir, f + "-shm")); } catch (_e) { /* cleanup — stale SHM may not exist */ }
  });
...
// Idempotency-Key cache (draft-ietf-httpapi-idempotency-key). Replay
// store for b.middleware.idempotencyKey; entries hold the captured
// response of a previously-completed mutating request so a retry from
// the same client (same Idempotency-Key + same body fingerprint)
// replays the original response without re-executing the handler.
// `body` is vault.seal'd since it may carry references to sealed data.
db.exec(`
...
`);
db.exec("CREATE INDEX IF NOT EXISTS idx_idempotency_keyHash ON idempotency_keys(keyHash)");
db.exec("CREATE INDEX IF NOT EXISTS idx_idempotency_expiresAt ON idempotency_keys(expiresAt)");
...
var COLUMNS = {
  ...
  idempotency_keys: ["_id", "keyHash", "fingerprint", "statusCode", "headers", "body", "createdAt", "expiresAt"],
};
...
module.exports = {
  ...
  idempotencyKeys: new Collection("idempotency_keys"),
};
