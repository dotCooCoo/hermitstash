  ...
  _merge(row) {
    ...
    for (var j = 0; j < this.jsonColumns.length; j++) {
      ...
      if (typeof doc[jc] === "string") {
        try { doc[jc] = JSON.parse(doc[jc]); } catch (_e) { /* leave as string if not valid JSON */ } // allow:bare-json-parse — parsing JSON-typed columns we wrote ourselves in this DB module
      }
    }
    return doc;
  }
