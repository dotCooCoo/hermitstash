...
// TOCTOU-safe read of an operator-writable PEM path: refuse a symlink source and
// any inode swap between the open and the read (CWE-367 / CWE-59), capped at 1 MiB.
// Matches the framework's own sealed-PEM reader (b.vault sealPemFile, 0.15.13).
function _safeReadPem(path, opts) {
...
function recoverFromMarker(markerPath, targetFilePath, otherFilePath) {
  var marker;
  try {
    marker = JSON.parse(nodeFs.readFileSync(markerPath, "utf8")); // allow:bare-json-parse — parsing migration marker we wrote ourselves in this same module
  } catch (e) {
...
function sealPemFile(plainPath, sealedPath, opts) {
  ...
  nodeFs.writeFileSync(tmp, sealed, { mode: 0o600 });
  _fsyncFile(tmp);
  ...
  nodeFs.writeFileSync(markerTmp, JSON.stringify(marker, null, 2), { mode: 0o600 });
  _fsyncFile(markerTmp);
  nodeFs.renameSync(markerTmp, markerPath);
  _fsyncDir(_parentDirOf(sealedPath));

  nodeFs.renameSync(tmp, sealedPath);
  _fsyncDir(_parentDirOf(sealedPath));
...
function unsealPemFile(sealedPath, plainPath) {
  ...
  nodeFs.writeFileSync(tmp, plain, { mode: 0o600 });
  _fsyncFile(tmp);
  ...
  nodeFs.renameSync(tmp, plainPath);
  _fsyncDir(_parentDirOf(plainPath));
