...
function sealPemFile(plainPath, sealedPath, opts) {
  ...
  var plain = nodeFs.readFileSync(plainPath);
...
function unsealPemFile(sealedPath, plainPath) {
  ...
  var sealed = nodeFs.readFileSync(sealedPath, "utf8").trim();
...
function loadPemDispatch(plainPath, sealedPath, modeEnvVarName) {
  ...
  if (hasSealed) {
    var raw = nodeFs.readFileSync(sealedPath, "utf8").trim();
    return Buffer.from(vault.unseal(raw), "utf8");
  }
  if (hasPlain) {
    return nodeFs.readFileSync(plainPath);
  }
