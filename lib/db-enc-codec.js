...
// Non-destructive probe: does `key` decrypt the existing db.enc bytes? Tries the
// AAD-bound read first, then the un-bound fallback (same dispatch as decryptDbEnc),
// returning a boolean instead of throwing. Used by the boot fail-closed guard to
// confirm a freshly minted DB key can read db.enc BEFORE it overwrites the only
// sealed copy of the real key.
function canDecryptDbEnc(packed, key, dir) {
  try { decryptDbEnc(packed, key, dir); return true; }
  catch (_e) { return false; }
}

module.exports = { decryptDbEnc: decryptDbEnc, encryptDbEnc: encryptDbEnc, canDecryptDbEnc: canDecryptDbEnc };
