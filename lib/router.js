/**
 * Custom HTTP router — zero-dependency replacement for express/koa.
 *
 * Why rolled-our-own: CLAUDE.md Rule 1 forbids npm runtime dependencies.
 * This router is ~200 lines, covers everything we need (path params,
 * middleware chain, static file serving, MIME sniffing), and leaves no
 * attack surface we haven't read. Feature parity with express is a
 * non-goal — add only what a route concretely requires.
 *
 * Middleware / handler dispatch (CLAUDE.md "Middleware signature"):
 *   - handler.length >= 3 → treated as middleware. Chain stops unless
 *     the handler calls next(). Using 2-arg handlers as middleware is
 *     structurally fragile and will silently fall through.
 *   - handler.length <= 2 → terminal handler. Always falls through to
 *     the next entry in the chain if it doesn't end the response.
 *
