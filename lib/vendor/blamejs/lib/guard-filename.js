...
function _detectIssues(input, opts) {
  ...
  codepointClass.detectCharThreats(name, opts, "filename")
 ...
 * path-traversal raw and percent-encoded, null-byte, UNC paths,
 * overlong UTF-8 sequences, and post-strip length-cap violation.
 * These classes are unrepairable, and silently fixing one would mask
 * the attack signal an audit log needs.
 *
 * NTFS alternate data streams are not in that floor. `adsPolicy`
 * (default `reject`) refuses them. At the permissive profile
 * `reservedCharPolicy` is `strip`, so a colon becomes `_` before the
 * ADS check runs and sanitize returns the repaired name instead of
 * throwing. `validate` reports `ntfs-ads` at every profile and `gate`
 * refuses it.
 *
 * @opts
 *   profile:    "strict"|"balanced"|"permissive",
