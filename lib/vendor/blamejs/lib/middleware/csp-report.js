...
function create(opts) {
  ...
  return async function cspReport(req, res, _next) {
    if (req.method !== "POST") {
      res.writeHead(C.HTTP.STATUS.METHOD_NOT_ALLOWED, { "Allow": "POST" });
      res.end();
    ...
    try {
      ...
      res.writeHead(C.HTTP.STATUS.CONTENT_TOO_LARGE);
      res.end();
    ...
    try { parsed = safeJson.parse(body.toString("utf8")); }
      ...
      res.writeHead(C.HTTP.STATUS.BAD_REQUEST);
      res.end();
    ...
    res.writeHead(C.HTTP.STATUS.NO_CONTENT);
    res.end();
  };
}
