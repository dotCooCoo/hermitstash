 ...
 *
 * On the HTTP backends that code means "the object endpoint answered 404",
 * which a provider also returns when the container or bucket itself is missing
 * or misconfigured — a HEAD carries no body and the error carries no response
 * headers, so the two are not distinguishable here. A `false` from this call
 * therefore means "the object is not readable at that path", not "the store is
 * healthy and the object is absent". Check the container separately with the
 * bucket operations if that difference matters.
 *
 * @opts
 *   classification:  string,    // route to a backend serving this classification
 *   backend:         string,    // explicit backend by name
