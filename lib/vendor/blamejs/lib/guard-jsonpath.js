...
function _countRecursiveDescents(s) {
  ...
  while (i + 1 < s.length) {
    var ch = s.charAt(i);
    if (ch === "'" || ch === "\"") { i = _skipQuoted(s, i); continue; }
    if (ch !== "." || s.charAt(i + 1) !== ".") { i += 1; continue; }
    var p = i + 2;
    if (p >= s.length) break;
    count += 1;
    i = p;
