...
function _selectedData(x509, selector) {
  if (selector === 0) return Buffer.from(x509.raw);
  if (selector === 1) return x509.publicKey.export({ format: "der", type: "spki" });
...
function matchCertificate(opts) {
  ...
  for (var i = 0; i < records.length; i++) {
    var rec = records[i];
    var eeUsage = rec.usage === 1 || rec.usage === 3;
    var certs = eeUsage ? [leaf] : [leaf].concat(chain);
