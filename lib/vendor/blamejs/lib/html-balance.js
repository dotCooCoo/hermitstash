 ...
 *   gates in one call.
 *
 *   Void elements (`<br>`, `<img>`, `<input>`, …) are recognized and
 *   never expected to close. Self-closing slash forms (`<foo />`)
 *   are honored. Raw-text elements skip their content entirely so
 ...
 * recognized; raw-text elements skip their bodies so `<script>` JS
...
module.exports = { check: check, checkSafe: checkSafe, VOID_TAGS: VOID_TAGS };
