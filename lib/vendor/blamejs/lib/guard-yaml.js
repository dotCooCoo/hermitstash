...
// No `g` flag, deliberately. This is only ever asked a yes/no question, and a
// global regex carries `lastIndex` between calls: `.test()` resumes from where
// the previous call stopped, so the same document answers true, then false,
// then true. Removing the flag makes that state impossible rather than
// requiring every caller to remember to reset it.
