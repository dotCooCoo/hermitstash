...
// Route prefixes that carry a single-use secret as the final path segment
// (password-reset / email-verify / invite tokens). The concrete token must never
// land in an audit row or be forwarded to a SIEM, so the trailing segment is
// templated to :token — the route pattern stays forensically useful while the
// secret is dropped (CWE-598, alongside the query-string strip below).
  ...
  function cleanup() {
    return auditArchive.runExclusivePurge(function () {
      try {
        ...
        // rows about to be deleted and advance the purge anchor to it — the
        ...
        // The never-lower helper keeps the anchor monotonic under any residual race.
        if (config.auditChainEnabled) {
