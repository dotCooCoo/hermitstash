/**
 * Every security-relevant action goes through audit.log().
 *
 * Addresses are stored as a one-way hash unless AUDIT_IP_FULL is on, in which
 * case the address itself is sealed and an investigator can read it back — for
 * entries written from then on. The user agent is captured only under
 * AUDIT_CAPTURE_USER_AGENT. Everything else identifying is sealed.
 */
...
// audit-archive requires audit lazily, so a top-of-file require here is safe.
var auditArchive = require("./audit-archive");
// audit-siem requires only config/b/logger (not audit) — safe top-of-file require.
var auditSiem = require("./audit-siem");
...
var ACTIONS = {
  ...
  AUDIT_ARCHIVE_VERIFIED: "audit_archive_verified",
...
// Routes carrying a single-use secret as their final path segment. The token
// must never reach an audit row or a SIEM, so the segment is templated to
// :token — the route stays forensically useful and the secret is dropped
// (CWE-598, with the query-string strip below).
var SECRET_PATH_PREFIXES = ["/auth/reset-password", "/auth/verify", "/auth/invite"];
function _templatizePath(p) {
  for (var i = 0; i < SECRET_PATH_PREFIXES.length; i++) {
    var pre = SECRET_PATH_PREFIXES[i];
    if (p.indexOf(pre + "/") === 0) return pre + "/:token";
  }
  return p;
}
...
// tokens/secrets — CWE-598); token-bearing path segments are templated before the
// bound so no fragment of the secret survives.
function _requestPath(req) {
  var p = req.pathname || req.path || (req.url ? String(req.url).split("?")[0] : null);
  if (!p) return null;
  return _templatizePath(String(p)).slice(0, MAX_CONTEXT.PATH);
}
...
// With the chain on, each row hashes the previous row's hash together with its
// own stored fields and a nonce, ordered by an integer counter. The framework
// owns that arithmetic; this module owns the append — seal first, so the chain
// hashes the at-rest representation, then insert — and the query callbacks the
// verifier reuses.
//
// Requests are served from one process, and the workers that do spawn write no
// audit rows, so serializing appends in memory is enough to keep read-tip,
// hash and insert atomic. Without it two concurrent appends read the same tip
// and fork the chain.
...
// The chain is opt-in, so a database that enabled it mid-life holds earlier
// rows with none of the chain columns set. Those are not chain members, but
// they sort first and the verifier would read a null previous hash as a break —
// so the read is scoped to chained rows and the walk starts at the real origin.
...
function log(action, opts) {
  ...
  // Forward to a SIEM in real time (no-op when disabled). Fire-and-forget; an
  // external SIEM outage never affects the local audit write below.
  auditSiem.forward(action, entry);
...
// The chain hashes a row's full physical column set, so widening that set means
// rows chained beforehand no longer verify: the verifier materialises the new
// columns as null and the canonical form stops matching the write-time hash.
// Moving the origin to the current tip once leaves those rows readable but
// below the anchor. Marker-guarded, and a no-op with no chained rows yet.
async function _reanchorChainForSchemaChange() {
  ...
  try {
    if (config.auditChainEnabled) {
      ...
      if (tip && tip.rowHash != null) {
        await auditArchive.upsertPurgeAnchorNeverLower(tip.monotonicCounter, tip.rowHash, "hs-audit-schema-evolution");
    ...
    try {
      // Atomic, symlink-refusing write (temp O_EXCL|O_NOFOLLOW + fsync + rename
      // + dir-fsync), matching how every other operator-writable path in this
      // tree is written. A raw writeFileSync follows a symlink planted at the
      // marker path, and DATA_DIR is the one directory an operator is expected
      // to hand to the container — the same reasoning that put O_NOFOLLOW on
      // the sealed-key reads and the logo fetch.
      b.atomicFile.writeSync(marker, new Date().toISOString(), { fileMode: 0o600 });
    } catch (markerErr) {
...
function startRetentionCleanup() {
  ...
  // Serialize the whole anchor-move + delete behind the shared purge mutex so it
  // can never interleave with an in-flight archival's _pruneArchived (which also
  // moves the anchor + deletes). Without the lock, retention could raise the anchor
  // to Y and delete [1..Y] while archival is mid-await, then archival lowers the
  // anchor back below Y → a permanent, self-perpetuating verifyChain false alarm.
  function cleanup() {
    return auditArchive.runExclusivePurge(async function () {
      try {
        var cutoff = new Date(Date.now() - days * TIME.days(1)).toISOString();

        // The origin has to move before the oldest rows go, or the chain breaks
        // at whichever row becomes the first. Rows are inserted in time order,
        // so the cutoff selects a contiguous prefix and its highest counter is
        // the new origin; the helper keeps the anchor from ever moving back.
        if (config.auditChainEnabled) {
          ...
          if (boundary && boundary.rowHash != null) {
            await auditArchive.upsertPurgeAnchorNeverLower(boundary.monotonicCounter, boundary.rowHash, "hs-retention-cleanup");
          }
        }
  ...
  // Run on start + every 24 hours. Both unref'd: the interval always was, and
  // the initial one was not, so requiring this module kept a process alive
  // until it elapsed. The server is unaffected — an unref'd timer still fires
  // while the listening socket keeps the loop running.
  var first = setTimeout(cleanup, TIME.seconds(5));
...
process.nextTick(async function () {
  // Signing the purge anchor made the re-anchor async; it still has to finish
  // before retention starts, or retention reads an origin that is about to move.
  try { await _reanchorChainForSchemaChange(); } catch (_e) { /* never block boot on re-anchor */ }
  startRetentionCleanup();
  // Size-triggered encrypted archival (opt-in via config.auditArchiveEnabled).
  try { auditArchive.startAuditArchival(); } catch (e) { logger.error("[audit] archival scheduler failed to start", { err: e && e.message }); }
  // PQC-signed chain-tip checkpoints (runs when the tamper chain is enabled) — a
  // keyed anchor over the keyless row-hash chain so a full-chain rewrite is caught.
  try { auditArchive.startCheckpointing(); } catch (e) { logger.error("[audit] checkpoint scheduler failed to start", { err: e && e.message }); }
  // SIEM forwarding (opt-in via config.siemEnabled). initSiem() is async; let it
  // settle in the background so boot never blocks on the dispatcher.
  Promise.resolve().then(auditSiem.initSiem).catch(function (e) { logger.error("[audit] SIEM forwarding failed to start", { err: e && e.message }); });
});
