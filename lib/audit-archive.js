...
// The anchor is signed with the audit signing key. It is the one row in the
// chain that says rows are ALLOWED to be missing, so an unsigned one lets any
// statement that can write this table erase a span of the trail and leave
// verifyChain clean. b.auditChain.verifyPurgeAnchor reports "unsigned" for a row
// with no signature and verifyChain refuses the chain, so signing here is what
// keeps retention and archival able to prune at all.
//
// Signing needs the key, so this is async. The three callers await it.
async function upsertPurgeAnchorNeverLower(counter, rowHash, bundleId, digests) {
  ...
  var d = digests || {};
  // Field-for-field what b.auditChain.purgeAnchorPayload canonicalises, with the
  // same defaults, so the bytes verification recomputes are the bytes signed
  // here. purgedAt is captured once and both signed and stored.
  var anchor = {
    scope: "audit",
    lastPurgedCounter: Number(counter),
    lastPurgedRowHash: String(rowHash),
    archiveBundleId: String(bundleId),
    purgedAt: Date.now(),
    firstPurgedCounter: d.firstPurgedCounter == null ? 0 : Number(d.firstPurgedCounter),
    archiveRowsDigest: d.archiveRowsDigest == null ? "" : String(d.archiveRowsDigest),
    archiveCheckpointDigest: d.archiveCheckpointDigest == null ? "" : String(d.archiveCheckpointDigest),
    archiveManifestDigest: d.archiveManifestDigest == null ? "" : String(d.archiveManifestDigest),
    fencingToken: 0,
  };
  var signature = b.auditSign.sign(b.auditChain.purgeAnchorPayload(anchor));

  db().rawExec(
    "INSERT INTO _blamejs_audit_purge_anchor (scope, lastPurgedCounter, lastPurgedRowHash, archiveBundleId, purgedAt, " +
    "firstPurgedCounter, archiveRowsDigest, archiveCheckpointDigest, archiveManifestDigest, signature, publicKeyFingerprint, fencingToken) " +
    "VALUES ('audit', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) " +
    ...
    "firstPurgedCounter = excluded.firstPurgedCounter, archiveRowsDigest = excluded.archiveRowsDigest, " +
    "archiveCheckpointDigest = excluded.archiveCheckpointDigest, archiveManifestDigest = excluded.archiveManifestDigest, " +
    "signature = excluded.signature, publicKeyFingerprint = excluded.publicKeyFingerprint, fencingToken = excluded.fencingToken",
    anchor.lastPurgedCounter, anchor.lastPurgedRowHash, anchor.archiveBundleId, anchor.purgedAt,
    anchor.firstPurgedCounter, anchor.archiveRowsDigest, anchor.archiveCheckpointDigest,
    anchor.archiveManifestDigest,
    Buffer.isBuffer(signature) ? signature : Buffer.from(signature),
    b.auditSign.getPublicKeyFingerprint(), anchor.fencingToken
  );
}
...
async function _archiveNowLocked(opts) {
  ...
  await _pruneArchived(rawRows, manifest);
...
async function _pruneArchived(rows, manifest) {
  if (manifest.chainEnabled && manifest.lastCounter != null && manifest.lastRowHash) {
    // Record the manifest digest inside the signed anchor so the archive that
    // opens this span is named by something the signature covers. Comparing it
    // against the bundle on disk is a verifier's job: b.auditChain.verifyChain
    // only does so when handed a resolveArchive callback, and the live chain
    // check passes none, so it verifies the digest rather than the archive.
    // verifyArchive checks a bundle's own signature and checksum directly.
    await upsertPurgeAnchorNeverLower(manifest.lastCounter, manifest.lastRowHash, manifest.id, {
      firstPurgedCounter: manifest.firstCounter,
      archiveManifestDigest: _manifestHash(manifest),
    });
  }
...
async function verifyArchive(id, passphrase) {
  ...
  if (manifest.chainEnabled) {
    ...
    // allowUnsignedPurgeAnchor applies to THIS walk only. The anchor above is
    // synthesized in memory from the manifest to give the slice its origin. It
    // is not a stored anchor and licenses nothing, and the manifest it is built
    // from is already covered by the bundle signature this function checks
    // before getting here. The live chain (app/domain/admin/audit.service.js)
    // passes no such option and still refuses an unsigned stored anchor.
    var walk = await b.auditChain.verifyChain(bundleQueryAll, "audit_log", { allowUnsignedPurgeAnchor: true });
    if (!walk.ok) {
...
function startAuditArchival() {
  ...
  // First check shortly after boot, then hourly. Both are unref'd so neither
  // holds the process open — the first one was not, and a timer that has to
  // elapse before the runtime will exit is a timer that holds it open for
  // exactly as long as it is set for. In the server nothing changes, because
  // an unref'd timer still fires while something else keeps the loop alive.
  var first = setTimeout(tick, C.TIME.seconds(30));
...
function startCheckpointing() {
  ...
  // Unref'd for the same reason as the archival check above: this one is set a
  // minute out, and un-unref'd it was the longest thing keeping any process
  // that merely required the audit module from exiting.
  var first = setTimeout(tick, C.TIME.minutes(1));
  if (first.unref) first.unref();
  var timer = setInterval(tick, C.TIME.hours(1));
  if (timer.unref) timer.unref();
}
...
module.exports = {
  ...
  // Verifying the chain now reads the purge anchor's signature, so a caller
  // that verifies without having archived first has to load the key itself.
  ensureSigning: _ensureSigning,
