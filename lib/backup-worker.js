  ...
  if (scope === "full") {
    ...
    try {
      ...
      if (encKeys.length > 0) {
        ...
        prevManifest = JSON.parse((await b.backupCrypto.decryptWithPassphrase(prevEnc, passphrase, prevHeader.argon2Salt)).toString("utf8"));
  ...
  for (var i = 0; i < keepManifests.length; i++) {
    try {
      ...
      if (allKeys.indexOf(encKey) !== -1) {
        ...
        parsed = JSON.parse((await b.backupCrypto.decryptWithPassphrase(encData, passphrase, hdr.argon2Salt)).toString("utf8"));
      } else {
