...
var { encryptVaultKey, TLS_FILES } = require("./backup-crypto");
...
// Backup-bucket key for an upload blob, content-addressed by its SHA3-512
// checksum. Distinct ciphertext never collides, so a rewrite-in-place (vault
// password rotation) produces a new immutable blob rather than overwriting the
// blob an older generation still references; identical bytes across generations
// dedup to one key. pruneBackups GCs this shared `backups/uploads/` namespace by
// the manifest-recorded s3Key, so reference-counting is unchanged.
function uploadBlobKey(checksum) {
  return "backups/uploads/" + checksum;
}

// ---- Walk uploads directory ----

async function walkUploads(dir, excludeAbs) {
  var results = [];
  try {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    if (entries[i].isDirectory()) {
      // Skip the plaintext chunk scratch directory (default <uploadDir>/chunks):
      // pre-assembly upload chunks are NOT app-encrypted and must never be copied
      // to the untrusted backup backend.
      if (excludeAbs && nodePath.resolve(full) === excludeAbs) continue;
      var sub = await walkUploads(full, excludeAbs);
...
async function runBackup() {
  ...
  // 1. Encrypt vault.key with passphrase. In wrapped mode, workerData
  // carries the in-memory key JSON because the plaintext file doesn't
  // exist on disk; the fallback path (disk read) is kept for plaintext
  // mode backward compatibility.
  var vaultResult = await encryptVaultKey(passphrase, DATA_DIR, workerData.vaultKeyJson);
  manifest.argon2Salt = vaultResult.salt;

  await backend.put(prefix + "vault.key.enc", vaultResult.encrypted);
  manifest.files["vault.key.enc"] = { s3Key: prefix + "vault.key.enc", size: vaultResult.encrypted.length, checksum: b.backupCrypto.checksum(vaultResult.encrypted) };
  ...
  try {
    ...
    manifest.files["db.key.enc"] = { s3Key: prefix + "db.key.enc", size: dbKeyBuf.length, checksum: b.backupCrypto.checksum(dbKeyBuf) };
    manifest.stats.dbFiles++;
    manifest.stats.totalSize += dbKeyBuf.length;
  } catch (_e) { /* db.key.enc may not exist */ }

  // 3. Upload hermitstash.db.enc. Preferred source is the in-memory snapshot
  // captured in the parent via db.snapshotEncryptedDb() — this guarantees
  // the backup contains the CURRENT committed state, not a stale version of
  // PATHS.dbEnc that might be up to 5 minutes out of date (periodic encrypt
  // interval) or missing entirely (ALLOW_DISK_DB mode has encPath=null).
  // Fall back to reading PATHS.dbEnc from disk only if the snapshot is
  // unavailable (defensive — shouldn't happen under normal flow).
  var dbEncBuf = workerData.dbEncSnapshot
    ? Buffer.from(workerData.dbEncSnapshot)
    : null;
  if (!dbEncBuf) {
    try { dbEncBuf = await fs.readFile(PATHS.dbEnc); } catch (_e) { /* db file may not exist */ }
  }
  if (dbEncBuf && dbEncBuf.length > 0) {
    await backend.put(prefix + "hermitstash.db.enc", dbEncBuf);
    manifest.files["hermitstash.db.enc"] = { s3Key: prefix + "hermitstash.db.enc", size: dbEncBuf.length, checksum: b.backupCrypto.checksum(dbEncBuf) };
    manifest.stats.dbFiles++;
  ...
  for (var t = 0; t < TLS_FILES.length; t++) {
    var tlsPath = nodePath.join(DATA_DIR, TLS_FILES[t].local);
    try {
      if (nodeFs.existsSync(tlsPath)) {
        var tlsBuf = await fs.readFile(tlsPath);
        var tlsEnc = await b.backupCrypto.encryptWithPassphrase(tlsBuf, passphrase, vaultResult.salt);
        await backend.put(prefix + TLS_FILES[t].key, tlsEnc);
        manifest.files[TLS_FILES[t].key] = { s3Key: prefix + TLS_FILES[t].key, size: tlsEnc.length, checksum: b.backupCrypto.checksum(tlsEnc) };
        manifest.stats.dbFiles++;
        manifest.stats.totalSize += tlsEnc.length;
      }
    } catch (_e) { /* optional file — skip if missing or unreadable */ }
  }
  ...
  if (scope === "full") {
    ...
    try {
      ...
      if (encKeys.length > 0) {
        ...
        var prevHeader = b.safeJson.parse((await backend.getBuffer(prevHeaderKey)).toString("utf8"));
        var prevEnc = await backend.getBuffer(encKeys[encKeys.length - 1]);
        prevManifest = b.safeJson.parse((await b.backupCrypto.decryptWithPassphrase(prevEnc, passphrase, prevHeader.argon2Salt)).toString("utf8"));
      } else if (plainKeys.length > 0) {
        var prevData = await backend.getBuffer(plainKeys[plainKeys.length - 1]);
        prevManifest = b.safeJson.parse(prevData.toString("utf8"));
      }
    ...
    if (workerData.storageBackend === "local") {
      // Local storage: walk filesystem, excluding the plaintext chunk scratch dir.
      var uploadDir = workerData.uploadDir;
      var chunkScratchAbs = workerData.chunkScratchDir ? nodePath.resolve(workerData.chunkScratchDir) : null;
      var files = await walkUploads(uploadDir, chunkScratchAbs);
      for (var i = 0; i < files.length; i++) {
        var relPath = nodePath.relative(uploadDir, files[i]).replace(/\\/g, "/");
        var fileBuf = await fs.readFile(files[i]);
        var checksum = b.crypto.sha3Hash(fileBuf);
        // Content-address the blob by its checksum, not the shared non-versioned
        // relPath. A vault password rotation rewrites a file's ciphertext in
        // place at the same relPath; a path-keyed blob would let the next backup
        // overwrite the blob an older generation still references, leaving that
        // older backup pointing at ciphertext its DB key can no longer
        // decapsulate. Content-addressing makes every distinct ciphertext an
        // immutable, independently-keyed blob — and gives cross-generation dedup
        // for free (identical bytes collapse to one key).
        var s3Key = uploadBlobKey(checksum);
  ...
  var manifestEnc = await b.backupCrypto.encryptWithPassphrase(manifestBuf, passphrase, vaultResult.salt);
  ...
  if (!b.crypto.timingSafeEqual(b.backupCrypto.checksum(verifyBuf), b.backupCrypto.checksum(manifestEnc))) {
  ...
  for (var i = 0; i < keepManifests.length; i++) {
    try {
      ...
      if (allKeys.indexOf(encKey) !== -1) {
        ...
        var hdr = b.safeJson.parse(headerData.toString("utf8"));
        var encData = await backend.getBuffer(encKey);
        parsed = b.safeJson.parse((await b.backupCrypto.decryptWithPassphrase(encData, passphrase, hdr.argon2Salt)).toString("utf8"));
      } else {
        var data = await backend.getBuffer(keepManifests[i]);
        parsed = b.safeJson.parse(data.toString("utf8"));
      }
      ...
      console.error("[backup] Pruning: could not decrypt manifest " + keepManifests[i] + " — keeping all uploads for safety"); // allow:console-direct — worker thread; no logger in worker context, parent reads stderr for operator visibility
      break;
    }
  }
