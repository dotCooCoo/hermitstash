...
// Bounds-checked 2-byte length read (mirrors blamejs's _envU16) — a bare
// packed.readUInt16BE(pos) on a truncated 0xE1 blob throws a raw Node
// RangeError that escapes the documented "Invalid envelope: ..." error
// contract. Untrusted at-rest ciphertext must fail closed with the typed
// envelope error, not an uncaught ERR_OUT_OF_RANGE.
function envU16(buf, at) {
  if (at + 2 > buf.length) throw new Error("Invalid envelope: truncated (expected a 2-byte length at offset " + at + ")");
  return buf.readUInt16BE(at);
}

// Bounds-checked subarray slice — a declared length that runs past the end of
// the buffer must surface the typed envelope error rather than silently feeding
// a clamped/short slice into decapsulate/diffieHellman.
function envSlice(buf, at, len) {
  if (at + len > buf.length) throw new Error("Invalid envelope: truncated (expected " + len + " bytes at offset " + at + ")");
  return buf.subarray(at, at + len);
}

function decryptEnvelope(packed, privateKeys) {
  if (!Buffer.isBuffer(packed) || packed.length < 4) throw new Error("Invalid envelope: too short (need at least the 4-byte suite header)");
  ...
  var kemCtLen = envU16(packed, pos); pos += 2;
  var kemCt = envSlice(packed, pos, kemCtLen); pos += kemCtLen;
  ...
  if (kemId === KEM.ML_KEM_1024_P384) {
    var ecEphLen = envU16(packed, pos); pos += 2;
    var ecEphDer = envSlice(packed, pos, ecEphLen); pos += ecEphLen;
  ...
  var nonce = envSlice(packed, pos, 24); pos += 24;
  // envSlice already guaranteed the 24-byte nonce fits, so `pos > packed.length`
  // could never fire (dead code). A Poly1305 AEAD needs at least a 16-byte tag
  // after the nonce; enforce that minimum so a truncated 0xE1 blob surfaces the
  // typed envelope error here rather than a raw cipher-library error downstream.
  if (pos + 16 > packed.length) throw new Error("Invalid envelope: truncated (ciphertext shorter than the 16-byte AEAD tag)");
