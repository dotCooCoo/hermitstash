...
function guard(opts) {
  ...
  return b.middleware.rateLimit(Object.assign({}, opts, {
    ...
    //
    // clientIp.rateKey keeps IPv4 exact but collapses IPv6 to its /64 (an
    // end-site rotating the low 64 bits of its allocation would otherwise mint a
    // fresh bucket per request and evade the throttle). The SAME helper backs
    // loginLimiter.reset() (see routes/auth.js) so keyFn and reset never diverge.
    keyFn: opts.keyFn || clientIp.rateKey,
