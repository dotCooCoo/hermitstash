...
// ---- b.middleware.rateLimit instance registry ----
//
// HS routes construct framework rate-limit middleware inline:
//   b.middleware.rateLimit({ scope: "2fa", max: 5, windowMs: ... })
// so the per-instance .reset(key) method is unreachable from outside the
// route file. Test harnesses (tests/helpers/test-server.js) need to clear
// all rate-limit windows between tests, otherwise a `verify` test that
// makes 5+ attempts trips the 2FA limiter and a downstream "single-use"
// check returns 429 instead of 401.
//
// Monkey-patch b.middleware.rateLimit to push every created instance
// onto a registry and expose resetAllInstances() so the test helper
// can flush them all in one call. Production never invokes the test
// helper, so the registry is dead weight at runtime — small, bounded,
// no measurable overhead.
var _b = require("./vendor/blamejs");
var _registeredInstances = [];
if (!_b.middleware._rateLimitRegistryWrapped) {
  var _origRateLimit = _b.middleware.rateLimit;
  _b.middleware.rateLimit = function (opts) {
    var instance = _origRateLimit(opts);
    _registeredInstances.push(instance);
    return instance;
  };
  _b.middleware._rateLimitRegistryWrapped = true;
}

function resetAllInstances(key) {
  for (var i = 0; i < _registeredInstances.length; i++) {
    try { _registeredInstances[i].reset(key); } catch (_e) { /* best-effort across backends */ }
  }
}
