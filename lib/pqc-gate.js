// codebase-patterns:allow-file raw-byte-literal — protocol-shape constants (TLS ClientHello byte offsets / record sizes)
/**
...
var PQC_GROUP_IDS = new Set(Object.values(b.constants.PQC_GROUPS));
...
var CLIENTHELLO_TIMEOUT = C.TIME.seconds(5);   // 5 seconds to receive ClientHello
...
function createPQCGate(internalPort) {
  var gate = net.createServer({ pauseOnConnect: true }, function (socket) {
    ...
    socket.on("data", function onData(chunk) {
      ...
      var buf = Buffer.concat(chunks); // allow:handrolled-buffer-collect — TCP-level ClientHello inspection peeks at chunks[0][0] before reassembly and enforces MAX_CLIENTHELLO_SIZE per-chunk; boundedChunkCollector's accumulate+result shape doesn't fit the incremental TLS-record-length parse
