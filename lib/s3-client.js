...
// Overall wall-clock cap per S3 op. Without it the sigv4 backend applies only an
// idle (zero-progress) timeout, so a peer that trickles one byte inside each idle
// window holds a backup/restore/download/migration request open indefinitely
// (blamejs 0.15.16 hardening). 120s suits the bulk backup/restore ops; callers can
// pass a lower opts.timeoutMs for interactive download.
var DEFAULT_S3_TIMEOUT_MS = C.TIME.seconds(120);
...
function S3Client(opts) {
  ...
  this._backend = b.objectStore.buildBackend({
    ...
    timeoutMs:       (opts.timeoutMs && opts.timeoutMs > 0) ? opts.timeoutMs : DEFAULT_S3_TIMEOUT_MS,
  });
}
