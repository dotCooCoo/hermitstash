 ...
 *   - HS:       `client.signPresigned(key, expires, { contentDisposition, contentType })` → URL string
 *     blamejs:  `backend.presignedDownloadUrl({ key, expiresIn, responseHeaders })` → `{ url, method, headers, expiresAt }`
 *
 * The pagination wrapper, presign-shape adapter, and `testConnection`
 * round-trip helper are HS-specific extras blamejs doesn't expose;
 * everything else is a thin name-translation.
 */
...
// Pre-signed download URL with `response-content-disposition` set to a
// caller-supplied filename. Delegates to b.objectStore.sigv4's
// presignedDownloadUrl (≥ 0.8.56) with the camelCase responseHeaders
// opt; flatten the `{ url, ... }` return to a plain URL string for HS's
// existing callers.
S3Client.prototype.signPresigned = function (key, expires, responseHeaders) {
  var rh = null;
  if (responseHeaders) {
    rh = {};
    if (responseHeaders.contentDisposition) rh.contentDisposition = responseHeaders.contentDisposition;
    if (responseHeaders.contentType)        rh.contentType        = responseHeaders.contentType;
  }
  var result = this._backend.presignedDownloadUrl({
    key:             key,
    // allow:raw-time-literal — S3 presign expiresIn is seconds (1h floor, 7d cap), not ms
    // allow:raw-byte-literal — same line, S3 second-count bounds
    expiresIn:       Math.min(Math.max(expires || 3600, 1), 604800),
    responseHeaders: rh,
  });
  return result.url;
};
