...
function recoverFromMarker(markerPath, targetFilePath, otherFilePath) {
  var marker;
  try { marker = JSON.parse(_safeReadRecovery(markerPath, { encoding: "utf8" })); } // allow:bare-json-parse — parsing migration marker we wrote ourselves in this same module
  catch (e) {
...
function loadKeysSync() {
  ...
  if (nodeFs.existsSync(PLAINTEXT_PATH)) {
    var loaded;
    try { loaded = JSON.parse(_safeReadRecovery(PLAINTEXT_PATH, { encoding: "utf8" })); } // allow:bare-json-parse — parsing vault keypair we wrote ourselves in this same module
    catch (e) {
    ...
    // Minting a brand-new vault keypair is correct ONLY on a true first run.
    // If db.key.enc or hermitstash.db.enc already exists, those were sealed /
    // encrypted with a vault keypair that is now MISSING — a lost or mismatched
    // data/vault.key, not a fresh install. Generating a new keypair here would
    // let lib/db.js unseal db.key.enc with the wrong key, fail, and (absent its
    // own fail-closed guard) overwrite the only sealed copy of the real DB key
    // — IRREVERSIBLE data loss. Refuse to start so the operator can restore the
    // matching vault.key.
...
async function init() {
  ...
  if (mode === "required") {
    ...
    if (hasSealed) {
      ...
      try { keys = JSON.parse(plaintextBuf.toString("utf8")); } // allow:bare-json-parse — parsing vault keypair JSON we wrote ourselves before sealing
      catch (_e) {
