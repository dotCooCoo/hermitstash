...
function loadKeysSync() {
  ...
  if (nodeFs.existsSync(PLAINTEXT_PATH)) {
    ...
    nodeFs.writeFileSync(PLAINTEXT_PATH, JSON.stringify(keys, null, 2), { mode: 0o600 });
...
async function init() {
  ...
  if (mode === "required") {
    ...
    if (hasSealed) {
      ...
      nodeFs.writeFileSync(SEALED_TMP_PATH, sealed, { mode: 0o600 });
      try {
        var fd = nodeFs.openSync(SEALED_TMP_PATH, "r+");
        try { nodeFs.fsyncSync(fd); } finally { nodeFs.closeSync(fd); }
