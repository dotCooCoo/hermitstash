...
function recoverFromMarker(markerPath, targetFilePath, otherFilePath) {
  var marker;
  try { marker = JSON.parse(fs.readFileSync(markerPath, "utf8")); } // allow:bare-json-parse — parsing migration marker we wrote ourselves in this same module
  catch (e) {
...
function loadKeysSync() {
  ...
  if (fs.existsSync(PLAINTEXT_PATH)) {
    var loaded;
    try { loaded = JSON.parse(fs.readFileSync(PLAINTEXT_PATH, "utf8")); } // allow:bare-json-parse — parsing vault keypair we wrote ourselves in this same module
    catch (e) {
