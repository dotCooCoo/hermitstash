 ...
 */
var fs = require("fs");
var { generateEncryptionKeyPair, sha3Hash } = require("./crypto");
var hermitstashCrypto = require("./crypto");
// Phase 2 swap — vault.seal always produces blamejs's 0xE2 envelope
// (NIST SP 800-56C r2 / RFC 9180 FixedInfo binding). vault.unseal
// dispatches on the envelope magic byte: 0xE2 → blamejs decrypt;
// 0xE1 → HermitStash's legacy decrypt (transitional, lets lib/db.js
// load db.key.enc during the boot-time auto-migrate window before
// every blob has been converted). After migration runs, the 0xE1
// branch is dead code; Phase 3 cleanup deletes the legacy import +
// dispatch when lib/crypto.js itself swaps to b.crypto.
var b = require("./vendor/blamejs");
var encrypt = b.crypto.encrypt;
function decrypt(b64, keys) {
  // Probe envelope magic byte — first byte of the base64-decoded buffer.
  // 0xE2 = blamejs (post-migration). 0xE1 = HermitStash legacy (still
  // on disk during the boot-time migration window).
  var first = Buffer.from(b64.substring(0, 4), "base64")[0];
  if (first === 0xE2) return b.crypto.decrypt(b64, keys);
  return hermitstashCrypto.decrypt(b64, keys);
}
...
async function initWrapped() {
  ...
  var plaintextJson;
  try {
    var plaintextBuf = await b.vaultWrap.unwrap(sealedBytes, passphrase);
...
async function initFirstRunWrapped() {
  ...
  var sealed;
  try {
    sealed = await b.vaultWrap.wrap(plaintextJson, passphrase);
  } catch (e) {
