...
function fire(eventName, payload) {
  ...
  for (var i = 0; i < hooks.length; i++) {
    ...
    (function (h, u) {
      isPrivateHost(u.hostname).then(function (priv) {
        if (priv) return;
        ...
        try {
          var req = https.request(h.url, { method: "POST", headers: { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(body), "X-Webhook-Signature": signature }, timeout: 5000 }, function() {});
