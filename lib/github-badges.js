...
async function fetchJson(path) {
  ...
  var ghToken = b.safeEnv.readVar("GITHUB_TOKEN");
  ...
  if (res.statusCode < 200 || res.statusCode >= 300) {
    throw new Error("github api " + path + " → HTTP " + res.statusCode);
  }
  var bodyText = res.body ? res.body.toString("utf8") : "";
  return b.safeJson.parse(bodyText);
}
