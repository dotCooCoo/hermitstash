...
function loadOrGenerate() {
  ...
  if (nodeFs.existsSync(KEYPAIR_PATH)) {
    ...
    var parsed;
    try {
      parsed = JSON.parse(json); // allow:bare-json-parse — JSON we wrote and sealed in this same module
    } catch (_e) {
      // json is the freshly-unsealed ML-KEM-1024 + P-384 API-encrypt keypair; a
      // SyntaxError here would carry raw key bytes in its message/stack (CWE-532),
      // and this call sits at server-main module scope with no local catch. Suppress
      // the parser detail — the on-disk keypair is corrupt if the parse fails.
      throw new Error("api-encrypt-keypair: on-disk keypair is not valid JSON (parser detail suppressed to avoid logging key material); restore " + KEYPAIR_PATH + " from backup");
    }
