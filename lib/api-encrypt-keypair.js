...
function _writeSealedAtomic(json) {
  ...
  nodeFs.writeFileSync(KEYPAIR_PATH_TMP, sealed, { mode: 0o600 });
  nodeFs.renameSync(KEYPAIR_PATH_TMP, KEYPAIR_PATH);
}
...
function loadOrGenerate() {
  ...
  if (nodeFs.existsSync(KEYPAIR_PATH)) {
    var sealed = nodeFs.readFileSync(KEYPAIR_PATH, "utf8");
