...
function forward(action, entry) {
  ...
  try {
    b.logStream.emit(_level(action), "audit." + action, {
      ...
      // details is free-form forensic context. The framework redactor is
      // name-based and cannot scan into a string value, and a shape-based mask is
      // not safe here: HS logs high-entropy content that is NOT secret (SHA3
      // checksums, storage paths) in the same shape as a token, so masking by
      // entropy would strip legitimate audit context. The control is at the
      // source — call sites log key PREFIXES / ids and reasons, never a raw
      // secret, in details (the same discipline lib/audit.js documents); the
      // tamper-evident audit log itself carries these identical values.
