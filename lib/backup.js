...
// The chunk scratch directory holds PLAINTEXT, pre-assembly upload chunks and
// defaults to <uploadDir>/chunks — inside the tree the backup walk scans. Hand it
// to the worker so the walk excludes it; a stored blob is app-encrypted before it
// reaches the (untrusted) backend, but a scratch chunk is not.
function resolveChunkScratchDir() {
  return storageLazy().scratchDir;
}
...
async function runBackup(passphrase) {
  ...
  // The entire lock-holding region runs inside try/finally so the lock is
  // released unconditionally. The pre-worker strands (getKeysJson /
  // snapshotEncryptedDb) can throw — getKeysJson throws while the vault
  // singleton is mid-reinit, snapshotEncryptedDb reads the DB file off disk
  // and can hit a transient EACCES/EIO/ENOENT. If those throws escaped with
  // the lock still latched, every subsequent backup AND restore would
  // short-circuit on the in-progress guard until process restart.
  var msg;
  try {
    // Emit the STARTED audit inside the guarded region so the lock's finally
    // always covers it (audit.log is throw-safe today; keeping it under the
    // try/finally removes the cross-module coupling on that guarantee).
    ...
    try {
      msg = await b.backup.runInWorker({
        ...
        // Overall wall-clock cap so a stalled worker (a hung S3 upload, a frozen
        // encrypt) is terminated and rejected instead of pinning _operationRunning
        // until the process restarts. Operator-tunable via config.backup.timeoutMs.
        timeoutMs: (config.backup && config.backup.timeoutMs) || C.TIME.minutes(30),
        args: {
          ...
          chunkScratchDir: resolveChunkScratchDir(),
          version: C.version,
        },
      });
    } catch (err) {
...
async function runRestore(passphrase, timestamp, opts) {
  ...
  // The lock-holding region runs inside try/finally so the lock is released
  // unconditionally. getCurrentPassphrase() (wrapped mode) runs before the
  // worker await and can throw while the vault singleton is mid-reinit; if
  // that throw escaped with the lock still latched, every subsequent backup
  // AND restore would short-circuit on the in-progress guard until restart.
  var msg;
  try {
    // Emit the STARTED audit inside the guarded region so the lock's finally
    // always covers it.
    ...
    try {
      msg = await b.backup.runInWorker({
        ...
        // Overall wall-clock cap so a stalled restore worker is terminated and
        // rejected rather than pinning _operationRunning until restart.
        timeoutMs: (config.backup && config.backup.timeoutMs) || C.TIME.minutes(30),
        args: {
          passphrase: passphrase,
          timestamp: timestamp,
  ...
  // A partial upload-restore that already triggered worker-side rollback
  // posts type:"error" (handled above). A type:"success" can still carry a
  // non-zero failedUploads only in dry-run preview, where nothing was
  // written — surface it through the return so the HTTP response + audit
  // detail report it instead of dropping the warning on the floor.
  var failedUploads = (msg.stats && msg.stats.failedUploads) || 0;
  audit.log(audit.ACTIONS.RESTORE_COMPLETED, {
    ...
    details: "Restore completed: " + (msg.stats ? msg.stats.dbFiles + " db files, " + msg.stats.uploadFiles + " upload files" : "success") +
      (failedUploads > 0 ? " (" + failedUploads + " upload files failed)" : ""),
  });
  return { success: true, stats: msg.stats, failedUploads: failedUploads, warning: msg.warning || null };
}
...
// Every tick of the scheduled job records its outcome here, with a reason, so
// the admin backup view can report it. Without this a backup could no-op
...
function getLastBackupAttempt() {
  ...
  return b.safeJson.parseOrDefault(row.value, null);
}
