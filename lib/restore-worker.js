...
var { decryptVaultKey, TLS_FILES } = require("./backup-crypto");
...
// Progress is logged to stdout — parent doesn't stream it anywhere
// (the admin endpoint awaits the whole restore and returns one final
// JSON response). Keeping the call sites as a single helper so the
// operator-visible log line is consistent.
function progress(step, detail, pct) {
  console.log("[restore]", step, "(" + pct + "%):", detail); // allow:console-direct — worker thread; no logger context, parent reads stdout for operator visibility
}
...
async function runRestore() {
  ...
  for (var i = 0; i < manifestKeys.length; i++) {
    try {
      ...
      var header = b.safeJson.parse(headerBuf.toString("utf8"));
  ...
  var manifestJson;
  try {
    manifestJson = await b.backupCrypto.decryptWithPassphrase(encManifestBuf, passphrase, targetHeader.argon2Salt);
  } catch (err) {
  ...
  var manifest = b.safeJson.parse(manifestJson.toString("utf8"));
  ...
  // Verify checksum
  if (!b.crypto.timingSafeEqual(b.backupCrypto.checksum(vaultEncBuf), String(vaultEntry.checksum || ""))) {
  ...
  if (!DRY_RUN && manifest.storageBackend === "s3" && manifest.uploads && Object.keys(manifest.uploads).length > 0 && !workerData.s3StorageConfig) {
  ...
  // Set once the first upload blob has been overwritten in place. Gates the
  // rollback in the catch: reverting the DB/vault after a blob was already
  // overwritten would strand that blob (its per-file key lives in the now-reverted
  // DB). Declared outside the try so the catch can read it.
  var uploadsMutated = false;

  // Everything from step 3 to step 6 is wrapped in a try/catch so a failure BEFORE
  // any upload blob is overwritten triggers a full rollback from the pre-restore
  // snapshots.
  try {
  ...
  if (dbKeyEntry) {
    ...
    if (!b.crypto.timingSafeEqual(b.backupCrypto.checksum(dbKeyBuf), String(dbKeyEntry.checksum || ""))) {
      throw new Error("db.key.enc checksum mismatch — backup may be corrupt");
    }
    if (!DRY_RUN) {
      // Atomic, symlink-refusing write (temp O_EXCL|O_NOFOLLOW + fsync + rename + dir-fsync).
      await b.atomicFile.write(PATHS.dbKeyEnc, dbKeyBuf, { fileMode: 0o600 });
    }
  }

  // 4. Download hermitstash.db.enc
  progress("database", "Downloading database...", 40); // allow:raw-byte-literal — UI progress percentage, not bytes
  var dbEncEntry = manifest.files["hermitstash.db.enc"];
  if (dbEncEntry) {
    var dbEncBuf = await backend.getBuffer(dbEncEntry.s3Key);
    if (!b.crypto.timingSafeEqual(b.backupCrypto.checksum(dbEncBuf), String(dbEncEntry.checksum || ""))) {
      throw new Error("hermitstash.db.enc checksum mismatch — backup may be corrupt");
    }
    if (!DRY_RUN) {
      await b.atomicFile.write(PATHS.dbEnc, dbEncBuf, { fileMode: 0o600 });
    }
  }
  ...
  if (!DRY_RUN) {
    if (workerData.vaultPassphraseMode === "required") {
      ...
      var sealedBytes = await b.vaultWrap.wrap(Buffer.from(vaultKeyJson, "utf8"), workerData.currentVaultPassphrase);
      // Atomic, symlink-refusing write. If a plaintext vault.key was left over
      // from a prior state, the boot state machine will flag the both-exist
      // invariant — in restore we don't clean that up automatically because it's
      // indicative of operator intervention we shouldn't silently override.
      await b.atomicFile.write(nodePath.join(DATA_DIR, "vault.key.sealed"), sealedBytes, { fileMode: 0o600 });
    } else {
      progress("vault", "Writing vault key...", 45);
      await b.atomicFile.write(PATHS.vaultKey, vaultKeyJson, { fileMode: 0o600 });
    }
  }
  ...
  for (var t = 0; t < TLS_FILES.length; t++) {
    ...
    if (tlsEntry) {
      try {
        ...
        var tlsPlain = await b.backupCrypto.decryptWithPassphrase(tlsEncBuf, passphrase, targetHeader.argon2Salt);
        if (!DRY_RUN) {
          var tlsLocalPath = nodePath.join(DATA_DIR, TLS_FILES[t].local);
          var tlsDir = nodePath.dirname(tlsLocalPath);
          if (!nodeFs.existsSync(tlsDir)) await fs.mkdir(tlsDir, { recursive: true });
          await b.atomicFile.write(tlsLocalPath, tlsPlain, { fileMode: 0o600 });
        } else {
  ...
  if (manifest.scope === "full" && uploadCount > 0) {
    ...
    for (var j = 0; j < uploadKeys.length; j++) {
      ...
      try {
        ...
        // Verify the blob against the manifest-recorded SHA3-512 checksum
        // BEFORE any path-safety check or write — so dry-run verifies too
        // (matching the worker's documented "checksum-verify everything"
        // promise) and so corrupt/truncated/tampered bytes never reach live
        // storage. The manifest is integrity-protected (decrypted with the
        // backup passphrase), so a missing/empty checksum signals a malformed
        // manifest, not a benign skip — fail closed.
        if (!b.crypto.timingSafeEqual(b.backupCrypto.checksum(fileBuf), String(uploadEntry.checksum || ""))) {
          throw new Error("upload checksum mismatch for " + relPath + " — backup may be corrupt or tampered");
        }
        ...
        if (restoreToS3) {
          // Validate the untrusted manifest relPath through the shared primitive
          // rather than a hand-rolled includes("..")/startsWith("/") check — an S3
          // key derived from a traversal / absolute path is refused.
          if (!b.safePath.validate(uploadDir, relPath).ok) {
            throw new Error("Rejected unsafe relative path: " + relPath);
          }
          if (!DRY_RUN) { await storageS3.put(relPath, fileBuf); uploadsMutated = true; }
        } else {
          // Resolve under uploadDir via b.safePath.resolve, which throws
          // safe-path/escapes-base or /absolute-rel on a traversal or absolute rel
          // (caught below and counted as a failed upload).
          var localPath = b.safePath.resolve(uploadDir, relPath);
          if (!DRY_RUN) {
            var dir = nodePath.dirname(localPath);
            if (!nodeFs.existsSync(dir)) await fs.mkdir(dir, { recursive: true });
            // Atomic, symlink-refusing write (temp O_EXCL|O_NOFOLLOW + fsync + rename + dir-fsync).
            await b.atomicFile.write(localPath, fileBuf, { fileMode: 0o600 });
            // A live blob has now been overwritten in place — past the point where
            // a DB/vault rollback would be safe (see the catch below).
            uploadsMutated = true;
          }
        }
        restored++;
  ...
  // A partial upload-restore is a failed restore — throw so it does not report
  // success. Whether this rolls the DB/vault/TLS back is decided in the catch by
  // uploadsMutated: if the failure happened before any blob was overwritten (a
  // corrupt/missing blob is caught by its checksum BEFORE the write), rollback is
  // safe and restores the exact pre-restore state; once a blob has been
  // overwritten in place, rollback would strand it. Skipped in dry-run.
  if (!DRY_RUN && failedUploads > 0) {
    throw new Error(failedUploads + " of " + uploadCount + " upload files could not be restored — backup may be corrupt or the backup bucket unreachable");
  }

  } catch (err) {
    // Roll back ONLY if no upload blob was overwritten yet. Steps 3-5 (db/vault/
    // TLS) each have a .pre-restore snapshot, so reverting them returns the server
    // to its exact pre-restore state — correct for any atomic-phase failure and for
    // an upload failure caught before the first write (e.g. a corrupt blob whose
    // checksum is verified pre-write). But once a blob is overwritten in place
    // (too large to snapshot), reverting the DB/vault would strand that blob under
    // a per-file key the reverted DB no longer holds (undecryptable); in that case
    // leave the restored state committed and report so the operator re-runs to
    // finish. The .pre-restore files are preserved on failure for inspection.
    if (!DRY_RUN && !uploadsMutated) {
  ...
  // Full success — clear the pre-restore snapshots.
  if (!DRY_RUN) {
