...
async function runRestore() {
    ...
    for (var j = 0; j < uploadKeys.length; j++) {
      ...
      try {
        ...
        if (restoreToS3) {
          ...
          var localPath = nodePath.join(uploadDir, relPath);
          var resolved = nodePath.resolve(localPath);
          if (!resolved.startsWith(nodePath.resolve(uploadDir) + nodePath.sep)) {
  ...
  // A partial upload-restore is a failed restore: the live DB/key/vault have
  // already been committed (steps 3-5) and now reference upload blobs that
  // failed to download or write (broken downloads, checksum mismatches,
  // backend blips). Throwing here — still inside the outer try — drives the
  // rollback path below, reverting db.key.enc/db.enc/vault to their
  // pre-restore state so the operator is never left with a committed DB
  // pointing at missing/stale files reported as success. Skipped in dry-run,
  // where nothing was written and failedUploads is surfaced as a warning only.
