...
async function runRestore() {
  ...
  if (dbKeyEntry) {
    ...
    if (!DRY_RUN) {
      var dbKeyTmp = nodePath.join(DATA_DIR, "db.key.enc.tmp");
  ...
  if (dbEncEntry) {
    ...
    if (!DRY_RUN) {
      var dbEncTmp = nodePath.join(DATA_DIR, "hermitstash.db.enc.tmp");
  ...
  if (!DRY_RUN) {
    if (workerData.vaultPassphraseMode === "required") {
      ...
      var sealedTmp = nodePath.join(DATA_DIR, "vault.key.sealed.tmp");
      ...
      await fs.rename(sealedTmp, nodePath.join(DATA_DIR, "vault.key.sealed"));
    } else {
      progress("vault", "Writing vault key...", 45);
      var vaultTmp = nodePath.join(DATA_DIR, "vault.key.tmp");
