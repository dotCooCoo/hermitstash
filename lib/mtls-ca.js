...
function _resolveCaKeySealedMode() {
  ...
  return nodeFs.existsSync(C.PATHS.CA_KEY_SEALED) ? "required" : "disabled";
}
...
module.exports = b.mtlsCa.create({
  ...
  // Operator escape hatch: MTLS_CA_ALGORITHM=ECDSA-P384-SHA384 pins the sync CA
  // classical (for peers that can't verify ML-DSA in TLS) and suppresses the
  // auto-migration to the ML-DSA-87 PQC default. Unset ⇒ the CA follows its stored
  // algorithm; the migration flips an existing classical CA to PQC by regenerating
  // through the engine (lib/mtls-migrate.js), not through this pin. A pin that
  // disagrees with an already-stored CA is refused by b.mtlsCa (rotate to change).
  algorithm:       process.env.MTLS_CA_ALGORITHM || undefined,
  vault:           vault,
});
