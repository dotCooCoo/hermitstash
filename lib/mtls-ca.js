 ...
 *
 * Uses OpenSSL CLI via child_process.execFileSync for certificate operations.
 * ECDSA P-384 for signatures (PQC cert signatures planned when OS TLS stacks support ML-DSA).
 */
var fs = require("fs");
...
var CA_KEY_PATH = process.env.MTLS_CA_KEY || path.join(DATA_DIR, "ca.key");
var CA_CERT_PATH = process.env.MTLS_CA_CERT || path.join(DATA_DIR, "ca.crt");
...
function initCA() {
  ...
  try {
    ...
    execFileSync("openssl", ["ecparam", "-genkey", "-name", "secp384r1", "-noout", "-out", CA_KEY_PATH], { stdio: "pipe" });
    fs.chmodSync(CA_KEY_PATH, 0o600);

    // Self-sign the CA certificate (10 year validity)
    execFileSync("openssl", ["req", "-new", "-x509", "-key", CA_KEY_PATH, "-out", CA_CERT_PATH, "-days", "3650", "-subj", "/CN=HermitStash Sync CA/O=HermitStash"], { stdio: "pipe" });
 ...
 */
function generateClientCert(cn, validityDays) {
  ...
  var tmpDir = path.join(DATA_DIR, "tmp-cert-" + Date.now());
  try {
    ...
    // Generate client key
    execFileSync("openssl", ["ecparam", "-genkey", "-name", "secp384r1", "-noout", "-out", clientKeyPath], { stdio: "pipe" });

    // Generate CSR
    execFileSync("openssl", ["req", "-new", "-key", clientKeyPath, "-out", clientCsrPath, "-subj", "/CN=" + cn + "/O=HermitStash Sync Client"], { stdio: "pipe" });

    // Sign with CA
    execFileSync("openssl", ["x509", "-req", "-in", clientCsrPath, "-CA", CA_CERT_PATH, "-CAkey", CA_KEY_PATH, "-CAcreateserial", "-out", clientCertPath, "-days", String(validityDays)], { stdio: "pipe" });
