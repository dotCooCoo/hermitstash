 ...
 * dependency. Works in any Node 24.14.1+ environment (uses node:crypto.webcrypto
 ...
 *   - SLH-DSA-SHAKE-256f or ML-DSA-87 cert signatures (post-quantum)
...
var CA_KEY_SEALED_PATH = _C.PATHS.CA_KEY_SEALED;
var pemSeal = require("./pem-seal");

// CA_KEY_SEALED env var:
//   "auto" or unset — load whichever exists (sealed if present, else plaintext)
//   "required"      — sealed file required; refuse to boot/operate on plaintext
//   "disabled"      — plaintext required; refuse to boot/operate on sealed
function caKeySealedMode() {
  return (process.env.CA_KEY_SEALED || "auto").toLowerCase();
}

// Load the CA private key — sealed or plaintext per the dispatch.
// Returns Buffer (PEM bytes) or null if neither file exists.
function loadCaKey() {
  return pemSeal.loadPemDispatch(CA_KEY_PATH, CA_KEY_SEALED_PATH, "CA_KEY_SEALED");
}

// True if a CA private key is available in either form.
function caKeyExists() {
  return fs.existsSync(CA_KEY_PATH) || fs.existsSync(CA_KEY_SEALED_PATH);
}
...
var pki = require("./vendor/blamejs/lib/vendor/pki.cjs");
var x509 = pki.x509;
var pkijs = pki.pkijs;
...
//
// TODO(pqc-certs): switch to { name: "SLH-DSA-SHAKE-256f" } (matches the
//   project-wide signing default) or { name: "ML-DSA-87" } when all mainstream
//   browsers + OS cert stores can verify PQ signatures on client certificates.
//   As of 2026-04 none do — issuing PQ-signed certs today would break mTLS
//   handshake validation in every browser. Node 24.14.1+ webcrypto supports both
//   algorithms via OpenSSL 3.5; pkijs likely needs a version bump to accept
...
// PKCS#12 parameters.
//
// v1.9.5 attempted PBES2 + AES-256-GCM (RFC 9579 PKCS#12 v3 spirit) but
// reverted: pkijs 3.4.0 emits an AES-GCM AlgorithmIdentifier without the
// GCMParameters ASN.1 structure, which fails to import with current
// openssl 3.5.5 (and presumably every other 2026 cert importer). This is
// a vendored-library bug, not a HermitStash design choice. Tracked as a
// v1.9.6+ candidate: bump pkijs to a version that emits valid
// GCMParameters, then revisit. CBC stays — already strong (2M PBKDF2
// iters + HMAC-SHA-512 outer MAC) and universally importable.
//
// TODO(pkcs12-upgrade):
//   - PBES2 + AES-256-GCM content encryption — blocked on pkijs
//   - PBMAC1 outer MAC (RFC 9579 spec) — blocked on pkijs API
 ...
 *
 * Honors CA_KEY_SEALED=required: writes the new CA key directly to the sealed
 * file (vault-sealed in memory then written) rather than emitting plaintext to
 * disk. The plaintext PEM never touches the filesystem in wrapped-bootstrap.
 */
function commitNewCa(caCertPem, caKeyPem) {
  var sealed = caKeySealedMode() === "required";
  var keyDest = sealed ? CA_KEY_SEALED_PATH : CA_KEY_PATH;
  var keyTmp = keyDest + ".tmp";
  var certTmp = CA_CERT_PATH + ".tmp";
  if (sealed) {
    var vault = require("./vault");
    fs.writeFileSync(keyTmp, vault.seal(caKeyPem), { mode: 0o600 });
  } else {
...
async function initCA() {
  if (caKeyExists() && fs.existsSync(CA_CERT_PATH)) {
    var keyBuf;
    try {
      keyBuf = loadCaKey();
    } catch (e) {
      logger.error("[mTLS] Failed to load CA key", { error: e.message });
      return null;
    }
    return { key: keyBuf, cert: fs.readFileSync(CA_CERT_PATH) };
  }
  try {
    var fresh = await generateNewCaInMemory();
    commitNewCa(fresh.caCertPem, fresh.caKeyPem);
    var sealedNow = caKeySealedMode() === "required";
    logger.info("[mTLS] CA keypair generated (pure JS)", {
      keyPath: sealedNow ? CA_KEY_SEALED_PATH : CA_KEY_PATH,
      certPath: CA_CERT_PATH,
      sigAlg: CA_SIG_ALG,
      generation: CA_GENERATION,
      sealed: sealedNow,
    });
    return { key: loadCaKey(), cert: fs.readFileSync(CA_CERT_PATH) };
  } catch (e) {
 ...
 */
function caExists() {
  return caKeyExists() && fs.existsSync(CA_CERT_PATH);
}
