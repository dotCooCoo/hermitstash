...
function _syncHydrateFromDb() {
  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      // to env / default.
      var plain = row.value ? vault.unseal(row.value) : "";
      if (typeof plain === "string") overlay[row.key] = _strip(plain).trim();
      else overlay[row.key] = plain;
    }
    cfg.reload(overlay);
...
var settingsMap = {
  ...
  auditRetentionDays:    { env: "AUDIT_RETENTION_DAYS",     apply: (v) => { config.auditRetentionDays = parseInt(v, 10) || 0; }, restart: true },
  ...
  auditArchiveThresholdRows:  { env: "AUDIT_ARCHIVE_THRESHOLD_ROWS", apply: (v) => { config.auditArchiveThresholdRows = parseInt(v, 10) || 50000; } },
  ...
  backupSchedule:    { env: "BACKUP_SCHEDULE",          apply: (v) => { config.backup.schedule = parseInt(v, 10) || b.constants.TIME.days(1); }, restart: true },
...
function updateSettings(changes) {
  ...
  for (var [key, value] of Object.entries(changes)) {
    ...
    var mapping = Object.prototype.hasOwnProperty.call(settingsMap, key) ? settingsMap[key] : null;
    if (!mapping) continue;
