...
// All string values are trimmed to prevent trailing-whitespace bugs (e.g. S3 bucket name with trailing space → DNS failure).
...
// Invalidate the cached S3 client when S3 config changes at runtime (lazy to avoid circular require)
function _resetS3() { try { require("./storage").resetS3Client(); } catch (_e) {} }
...
const config = {
  ...
  allowedDomains: parseList(loadSetting("ALLOWED_DOMAINS", "")),
  adminEmails: parseList(loadSetting("ADMIN_EMAILS", "")),

  allowedExtensions: parseList(
    loadSetting("ALLOWED_EXTENSIONS",
...
const settingsMap = {
  ...
  s3Bucket:          { env: "S3_BUCKET",              apply: (v) => { config.storage.s3.bucket = v; _resetS3(); } },
  s3Region:          { env: "S3_REGION",              apply: (v) => { config.storage.s3.region = v; _resetS3(); } },
  s3AccessKey:       { env: "S3_ACCESS_KEY",          apply: (v) => { config.storage.s3.accessKey = v; _resetS3(); } },
  s3SecretKey:       { env: "S3_SECRET_KEY",          apply: (v) => { config.storage.s3.secretKey = v; _resetS3(); } },
  s3Endpoint:        { env: "S3_ENDPOINT",            apply: (v) => { config.storage.s3.endpoint = v; _resetS3(); } },
...
function updateSettings(changes) {
  ...
  for (const [key, value] of Object.entries(changes)) {
    ...
    var cleaned = typeof value === "string" ? value.trim() : String(value);
