 ...
 *
 * v1.9.39: backing store routed through b.config.create() with vault.unseal
 * as the transformValue. The framework owns the subscriber primitive
 * (cfg.subscribe), DB-row → config-overlay merging, and the schema-validated
 * value snapshot in cfg.value. HermitStash still owns the operator-facing
 * shape (the exported config object with nested google/storage/backup/email
 * groups and per-setting coercion) and the save path (validation,
 * vault-sealing on write, restart-required flag). The two communicate via
 * cfg.reload(overlay) on admin save and cfg.subscribe → _rebuildConfig on
 * every reload.
 */
...
// ---- b.config backing store ----
// Permissive record(string) schema: HermitStash applies per-setting coercion
// in the s/n/bool/bFalse helpers below, so cfg.value just needs to expose
// the raw string values it inherits from env + DB-overlay. The schema is
// load-bearing only because b.config.create requires one.
var cfg = b.config.create({
  schema: b.safeSchema.record(b.safeSchema.string()),
  env:    process.env,
});

// Synchronously hydrate cfg from the DB-stored settings (vault-unseal each).
// Tolerant of db-not-ready at module-load (cfg.value stays at env baseline
// and a later call — typically after db.init() lands — populates it).
function _syncHydrateFromDb() {
  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      // Row exists with explicit empty value = operator's choice (list-type
      // settings cleared via admin UI). Preserve it instead of falling back
      // to env / default.
      // row.value is already plaintext — the settings repo auto-unseals the
      // sealed `value` column on read (field-crypto: settings.seal = ["value"]).
      // A second unseal here is a no-op for ordinary plaintext but THROWS on an
      // operator value that happens to begin with the "vault:" sentinel, and the
      // throw is caught below and discards the ENTIRE overlay — every DB-stored
      // setting silently reverts to its env baseline. Use the unsealed value.
      var plain = row.value || "";
      // Proto-pollution-safe own-set: row.key is an operator-influenced settings key
      // from the DB, so a "__proto__"-named row can't re-parent the overlay object.
...
// Reverse map ENV var name -> settings-schema key, derived once from
// settingsMap (the single source of truth for the env<->key correspondence).
// Built lazily because settingsMap is defined below _build(); the first
// synchronous _build() runs after settingsMap is populated (see the relocated
// `var config = _build();` past the settingsMap definition).
...
// Load a setting: cfg.value (DB-overlay > env) then default.
// All string values are sanitized (trim + strip control chars) to prevent whitespace/injection bugs.
// A resolved value that fails its settings-schema bound (e.g. PORT=99999 over
// the 65535 max, BACKUP_TIMEZONE=America/Foobar) falls back to defaultVal here
// so the load path agrees with the schema instead of binding an out-of-range
// value at listen() time. settingsSchema.validate returns {valid:true} for
// unknown keys and for empty values, so non-schema settings pass through
// unchanged. This is also why validateEnvVars' "Using default." warning is
// now literally true.
...
// Wraps cfg.subscribe so the framework owns the subscriber array.
var _resetCallbacks = [];
var _lastChangedKeys = [];
function onReset(fn) { _resetCallbacks.push(fn); }
// Parse a strictly-positive integer, else fall back to `def`. A NEGATIVE value
// is truthy, so the old `parseInt(...) || def` let a stray `MAX_FILE_SIZE=-1`
// (or any negative size / count / duration) stick — the parser downstream then
// throws on a non-positive maxBytes and disables the whole upload/vault surface.
// A non-positive value is never valid for HS's numeric settings, so it resolves
// to the default. `pnum` is the shared coercion for both the load path (via `n`)
// and the hot-reload `apply` functions in settingsMap below.
...
// Module-load: pull DB rows once so config below reflects DB > env > default
// instead of env > default only.
_syncHydrateFromDb();

// Rebuilds + returns the operator-facing config object structure from
// current cfg.value. Called once at module load and again on every
// cfg.subscribe firing so the exported config object stays current after
// admin saves.
function _build() {
  // SIEM forwarding is the de-hash opt-in: a SIEM is only useful with the real
  // source IP and user-agent, so turning it on forces both audit-capture toggles
  // on. One switch governs visibility — the stored log and the forwarded stream
  // stay consistent, and an investigator never receives a hashed IP in their SIEM.
  ...
  tailscale: {
    ...
    // tailscaled LocalAPI socket. WhoIs-by-peer-IP over this socket is the
    // spoof-proof identity source (the Tailscale-User-* headers are a
    // convenience confirmed against it). In the bundled sidecar, tailscaled
    // runs in-container at this path.
    socketPath: s("TAILSCALE_SOCKET", "/run/tailscale/tailscaled.sock"),
    // Sign in with Tailscale (SSO). Requires `enabled`. A request arriving over
    // `tailscale serve` (loopback, carrying the trusted identity family) can
    // sign in; account creation is governed by the admin-gate below.
    ssoEnabled: bFalse("TAILSCALE_SSO"),
    // Admin-gated auto-provisioning. A tailnet user with no existing account is
    // auto-created ONLY when they carry the required capability grant (advertised
    // in the tailnet ACL, surfaced by WhoIs) OR their login is on the allowlist.
    // With BOTH empty, SSO signs in existing accounts only and never creates one
    // — the safe default for "SSO on, provisioning off".
    ...
    // Serve mode the opt-in boot-time sidecar install configures: "serve"
    // (tailnet-only, injects identity headers) | "funnel" (public internet, NO
    // identity) | "off" (operator runs their own tailscaled). Read by the
    // container entrypoint; the app reads it only to reflect its exposure.
  ...
  // Default OFF: the source IP is stored as a one-way hash the operator cannot
  // reverse (privacy-preserving). ON stores the full IP vault-sealed (reversible)
  // so investigations can read it — a deliberate privacy reduction, new rows only.
  // Forced ON while SIEM forwarding is enabled (the SIEM needs the real IP).
  ...
  // Tamper-evidence audit chain. Default OFF: enabling it changes the audit
  // write path from synchronous fire-and-forget to a serialized async append
  // (read chain tip → hash → insert) and adds a per-row SHA3-512 cost — a
  // perf trade-off the operator opts into, not a silent default. When on,
  // auditChainStrict additionally refuses to boot on a verify mismatch
  // (default: log at ERROR and continue).
  // Archival requires the chain (bundles carry chain proof), so enabling archival
  // forces the chain on here at construction — enforced server-side, not just by the
  // settings UI, so AUDIT_ARCHIVE via env or a direct API call also enables it.
  auditChainEnabled: bool("AUDIT_CHAIN", false) || bool("AUDIT_ARCHIVE", false),
  auditChainStrict: bool("AUDIT_CHAIN_STRICT", false),
  // Size-based encrypted archival. When enabled and audit_log exceeds the row
  // threshold, the oldest entries are written to a passphrase-encrypted, PQC-signed
  // bundle on disk and pruned (chain re-anchored). Off by default; requires the
  // chain + a passphrase. See lib/audit-archive.js.
  ...
  // SIEM forwarding — stream every audit event to a SIEM in real time via
  // RFC 5424 syslog or an HTTP webhook (Splunk HEC / Datadog / Loki / generic).
  // Off by default. Secrets/PII value-shapes are redacted before leaving the host.
  siemEnabled: siemOn,
  ...
  // Soft mTLS enforcement. When true, middleware/web-guard.js drops non-mTLS
  // connections (except /sync/*, /health, Bearer-authenticated calls, and the
  // enforce-mtls toggle endpoint itself) via socket.destroy() — no HTTP
  // response rendered. TLS handshake itself still completes because it's an
  // app-layer check.
  //
  // For hard enforcement at the TLS layer (rejectUnauthorized: true), set the
  // ENFORCE_MTLS_STRICT env var. See server.js for the boot-time resolution.
  // ENFORCE_MTLS_STRICT=false is the lockout escape hatch: it forces soft
  // enforcement off on EVERY rebuild, not just at boot, so a settings hot-reload
  // (which re-reads the DB-persisted ENFORCE_MTLS=true) cannot silently re-lock
  // the operator mid-session. The strict-strict resolution still lives in
  // server.js; this honors the same env override durably in the derived field.
  enforceMtls: process.env.ENFORCE_MTLS_STRICT === "false" ? false : bool("ENFORCE_MTLS", false),
  // Auto-migrate an existing classical (ECDSA-P384) sync CA to the ML-DSA-87
  // PQC default at boot when the runtime can verify ML-DSA in TLS. On by
  // default; set MTLS_AUTO_MIGRATE=false to keep a classical sync CA in place
  // (e.g. for peers that can't verify ML-DSA). See lib/mtls-migrate.js.
  ...
  email: {
    // Master kill switch — when false, every sendEmail() returns without
    // attempting delivery. All SMTP / Resend settings are preserved so the
    // admin can re-enable later without re-entering credentials. Lets
    // operators exercise the admin-side out-of-band delivery UX
    // (invite URLs, reset links, verification links) without touching backend
    // credentials, and provides a clean way to silence email during outages.
  ...
  // Opt-in CIDR fence for the /admin surface. Empty (default) = no fence —
  // /admin stays reachable from any network and requireAdmin auth is the only
  // gate. A non-empty list mounts an additive network-layer fence on top of
  // auth (server-main.js). loadListSetting so a cleared list persists as empty.
  adminAllowedCidrs: parseList(loadListSetting("ADMIN_ALLOWED_CIDRS", "")),
  setupComplete: bFalse("SETUP_COMPLETE"),
};}

// `var config = _build();` is hoisted below the settingsMap definition so the
// env->schema-key reverse map (envToSchemaKey) is populated before the first
// build runs — this lets loadSetting's schema-bound fallback cover boot-time
// env values (e.g. an out-of-range PORT) and not just hot-reloads.
var config;
...
var settingsMap = {
  ...
  port:              { env: "PORT",                  apply: (v) => { config.port = pnum(v, 3000); }, restart: true },
  sessionSecret:     { env: "SESSION_SECRET",        apply: (v) => { config.sessionSecret = v; }, restart: true },
  sessionIdleTimeout:{ env: "SESSION_IDLE_TIMEOUT",  apply: (v) => { config.sessionIdleTimeout = Math.max(pnum(v, 1800000), 60000); } },
  ...
  tailscaleEnabled:  { env: "TAILSCALE_ENABLED",     apply: (v) => { config.tailscale.enabled = v === "true"; }, restart: true },
  tailscaleSocket:   { env: "TAILSCALE_SOCKET",      apply: (v) => { config.tailscale.socketPath = v || "/run/tailscale/tailscaled.sock"; }, restart: true },
  tailscaleSso:      { env: "TAILSCALE_SSO",         apply: (v) => { config.tailscale.ssoEnabled = v === "true"; } },
  tailscaleSsoGrant: { env: "TAILSCALE_SSO_GRANT",   apply: (v) => { config.tailscale.ssoRequiredGrant = v; } },
  tailscaleSsoAllowlist: { env: "TAILSCALE_SSO_ALLOWLIST", apply: (v) => { config.tailscale.ssoAllowlist = parseList(v); } },
  tailscaleHostnameAutoConfig: { env: "TAILSCALE_HOSTNAME_AUTOCONFIG", apply: (v) => { config.tailscale.hostnameAutoConfig = v !== "false"; }, restart: true },
  tailscaleServeMode: { env: "TAILSCALE_SERVE_MODE", apply: (v) => { config.tailscale.serveMode = v || "off"; }, restart: true },
  ...
  maxFileSize:       { env: "MAX_FILE_SIZE",         apply: (v) => { config.maxFileSize = pnum(v, 104857600); } },
  uploadTimeout:     { env: "UPLOAD_TIMEOUT",        apply: (v) => { config.uploadTimeout = pnum(v, 300000); } },
  uploadConcurrency: { env: "UPLOAD_CONCURRENCY",    apply: (v) => { config.uploadConcurrency = pnum(v, 3); } },
  uploadRetries:     { env: "UPLOAD_RETRIES",         apply: (v) => { config.uploadRetries = pnum(v, 2); } },
  ...
  fileExpiryDays:    { env: "FILE_EXPIRY_DAYS",       apply: (v) => { config.fileExpiryDays = pnum(v, 0); } },
  storageQuotaBytes: { env: "STORAGE_QUOTA_BYTES",    apply: (v) => { config.storageQuotaBytes = pnum(v, 0); } },
  perUserQuotaBytes: { env: "PER_USER_QUOTA",         apply: (v) => { config.perUserQuotaBytes = pnum(v, 0); } },
  ...
  publicMaxFiles:    { env: "PUBLIC_MAX_FILES",       apply: (v) => { config.publicMaxFiles = pnum(v, 200); } },
  publicMaxBundleSize:{ env: "PUBLIC_MAX_BUNDLE_SIZE",apply: (v) => { config.publicMaxBundleSize = pnum(v, 524288000); } },
  publicIpQuotaBytes: { env: "PUBLIC_IP_QUOTA_BYTES",  apply: (v) => { config.publicIpQuotaBytes = pnum(v, 0); } },
  ...
  auditRetentionDays:    { env: "AUDIT_RETENTION_DAYS",     apply: (v) => { config.auditRetentionDays = pnum(v, 0); }, restart: true },
  auditIpFull:           { env: "AUDIT_IP_FULL",            apply: (v) => { config.auditIpFull = v === "true"; } },
  auditCaptureUserAgent: { env: "AUDIT_CAPTURE_USER_AGENT", apply: (v) => { config.auditCaptureUserAgent = v === "true"; } },
  ...
  // are wired at start).
  auditChainEnabled:          { env: "AUDIT_CHAIN",                 apply: (v) => { config.auditChainEnabled = v === "true"; }, restart: true },
  auditArchiveEnabled:        { env: "AUDIT_ARCHIVE",                apply: (v) => { config.auditArchiveEnabled = v === "true"; if (v === "true") config.auditChainEnabled = true; }, restart: true },
  auditArchiveThresholdRows:  { env: "AUDIT_ARCHIVE_THRESHOLD_ROWS", apply: (v) => { config.auditArchiveThresholdRows = pnum(v, 50000); } },
  auditArchivePassphrase:     { env: "AUDIT_ARCHIVE_PASSPHRASE",     apply: (v) => { config.auditArchivePassphrase = v; } },
  // SIEM forwarding — re-init the log-stream dispatcher on the next boot.
  siemEnabled:      { env: "SIEM_ENABLED",        apply: (v) => { config.siemEnabled = v === "true"; }, restart: true },
  siemProtocol:     { env: "SIEM_PROTOCOL",       apply: (v) => { config.siemProtocol = v || "syslog"; }, restart: true },
  siemUrl:          { env: "SIEM_URL",            apply: (v) => { config.siemUrl = v || ""; }, restart: true },
  siemWebhookAuth:  { env: "SIEM_WEBHOOK_AUTH",   apply: (v) => { config.siemWebhookAuth = v || "none"; }, restart: true },
  siemWebhookToken: { env: "SIEM_WEBHOOK_TOKEN",  apply: (v) => { config.siemWebhookToken = v; }, restart: true },
  siemMinLevel:     { env: "SIEM_MIN_LEVEL",      apply: (v) => { config.siemMinLevel = v || "info"; }, restart: true },
  ...
  backupSchedule:    { env: "BACKUP_SCHEDULE",          apply: (v) => { config.backup.schedule = pnum(v, b.constants.TIME.days(1)); }, restart: true },
  ...
  backupRetention:   { env: "BACKUP_RETENTION",         apply: (v) => { config.backup.retention = pnum(v, 7); } },
  ...
  emailEnabled:      { env: "EMAIL_ENABLED",          apply: (v) => { config.email.enabled = v !== "false"; } },
  enforceMtls:       { env: "ENFORCE_MTLS",           apply: (v) => { config.enforceMtls = v === "true"; } },
  mtlsAutoMigrate:   { env: "MTLS_AUTO_MIGRATE",      apply: (v) => { config.mtlsAutoMigrate = v !== "false"; } },
  ...
  resendQuotaDaily:  { env: "RESEND_QUOTA_DAILY",     apply: (v) => { config.email.resendQuotaDaily = pnum(v, 100); } },
  resendQuotaMonthly:{ env: "RESEND_QUOTA_MONTHLY",   apply: (v) => { config.email.resendQuotaMonthly = pnum(v, 3000); } },
  smtpHost:          { env: "SMTP_HOST",              apply: (v) => { config.email.host = v; } },
  smtpPort:          { env: "SMTP_PORT",              apply: (v) => { config.email.port = pnum(v, 587); } },
  ...
  adminAllowedCidrs: { env: "ADMIN_ALLOWED_CIDRS",   apply: (v) => { config.adminAllowedCidrs = parseList(v); }, restart: true },
...
function getSettings() {
  return {
    ...
    // Mask the passphrase the same way every other sensitive field is masked,
    // so the form pre-populates with bullets and the existing masked-skip
    // logic at the apply path preserves the value when the operator saves
    // unrelated backup settings (schedule, timezone, etc.) without re-typing
    // the passphrase. Without this, an empty form input clobbers the saved
    // value and silently breaks scheduled backups.
...
// Sensitive keys — only update if value doesn't look like a mask, AND
// always render as bullets in the admin Environment Overrides surface
// (lib/config.js getEnvironment). v1.9.5 audit added backupPassphraseHash:
// it's an Argon2 PHC string, not the raw passphrase, but exposing it lets
// an attacker do offline brute-force against the password. Mask it.
