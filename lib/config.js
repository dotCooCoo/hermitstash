...
var config = {
  ...
  enforceMtls: b("ENFORCE_MTLS", false),
...
function updateSettings(changes) {
  ...
  if (updated.length > 0) {
    for (var ri = 0; ri < _resetCallbacks.length; ri++) {
      try { _resetCallbacks[ri](updated); } catch (_e) { /* reset callback failure must not block settings save */ }
    }
  }
