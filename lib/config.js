...
// to the default. `pnum` is the shared coercion behind `n` on the load path.
...
function _build() {
  ...
  // Additional origins the app answers on besides rpOrigin — a LAN hostname, a
  // tailnet MagicDNS name. Accepted for state-changing requests alongside
  // rpOrigin; rpOrigin alone still generates every share link, email URL and
  // sitemap entry.
  //
  // Deliberately NOT corsOrigins. The two answer different questions: CORS
  // governs which origins may READ a response, this governs which may CAUSE a
  // state change. Folding one into the other would silently widen CSRF
  // acceptance for every deployment that set CORS_ORIGINS for unrelated reasons.
...
// Map of setting keys to the env var each one reads, plus whether changing it
// needs a restart. That is the whole contract — this map does not decide where
// a value lands on the config object. _build() above does, reading every value
// back out of cfg.value, and a save reaches it through _syncHydrateFromDb() →
// cfg.reload → subscribe → _build.
//
// Each entry used to carry an `apply` function that assigned the value to its
// place on the config object. Nothing called them: the rebuild replaced
// in-place mutation, and the functions were left behind, so a new setting could
// be given a hook that looked like the thing making it take effect and did
// nothing. Add a setting by giving it an entry here, a settings-schema type, a
// line in _build(), and a line in getSettings().
var settingsMap = {
