...
var FIELD_SCHEMA = {
  ...
  idempotency_keys: {
    // raw: keyHash (already a SHA3-512 hash, indexed for lookup), statusCode (HTTP status enum), createdAt/expiresAt (cleanup index)
    // body sealed because cached responses may carry references to sealed data; headers + fingerprint sealed defensively
    seal: ["fingerprint", "headers", "body"],
  },
};
...
function registerWithBlamejs() {
  for (var table in FIELD_SCHEMA) {
    ...
    b.cryptoField.registerTable(table, {
      sealedFields: Array.isArray(s.seal) ? s.seal : [],
    });
  }
}
