...
var FIELD_SCHEMA = {
  ...
  bundles: {
    ...
    // skippedFiles holds {path,reason} of skipped uploads — relative file paths are
    // content, not a raw/queryable field, so they seal like allowedEmails (both are
    // JSON-typed columns; _merge unseals before the JSON-column parse, see lib/db.js).
  ...
  audit_log: {
    // monotonicCounter / prevHash / rowHash / nonce stay RAW (not sealed): the
    // tamper-evidence chain recomputes rowHash over the stored row and orders
    // by the integer counter — sealing any of them would break verification.
    // raw: method (HTTP verb enum), authType (session|apikey|anonymous), requestId
    // (random correlation id) — not PII. path + userAgent carry identifiers → sealed.
    // NOTE: `ip` is sealed, but what gets sealed depends on AUDIT_IP_FULL — by
    // default it's a one-way hash of the IP (unsealing yields the hash, not the
    // address); with the flag on it's the full IP (unsealing yields the address).
    // See lib/audit.js log().
...
//
// Rows written before the key existed carry an unkeyed SHA3-512 namespaceHash,
// recomputable from the plaintext alone. derivedLegacy reproduces that digest so
// lookups dual-read both forms until the boot backfill
// (lib/derived-hash-backfill) rewrites those rows to the keyed digest, after
// which the legacy candidate matches nothing.
//
...
function sealDoc(table, doc, rowId) {
  ...
  // verbatim. Two cases:
  //   1. Non-string falsy (false / 0 / NaN): a few sealed columns hold the
  //      occasional boolean/number in practice (e.g. users.vaultEnabled), and an
  //      AEAD seal would coerce them to a string ("false") or otherwise change
  //      their type. Keep them raw so the type/value round-trips exact.
  ...
  //
  // An empty string is deliberately NOT passed through — it is sealed. A current
  // b.cryptoField encodes "" to a typed marker and seals it as a real
  // tamper-evident envelope that round-trips to "", and its read side fail-closes
  // (nulls + audits) a BARE "" found in an AAD-bound / per-row-key column (a
  // DB-write attacker's envelope-downgrade). Passing "" through raw would
  // therefore make it read back as null; sealing keeps both the round-trip and
  // the tamper-evidence. (Any pre-existing raw-"" cell reads back null under that
  // same read-side fail-closed rule regardless of what this write path does.)
  ...
  if (schema.derived) {
    for (var dk in schema.derived) {
      if (schema.derived[dk].from === field) {
        var def = schema.derived[dk];
        // Mirror the derived-index WRITE contract (a falsy source is stored as a
        // null index, never a digest — an always-hash would collide across every
        // empty-source row on a unique index). A lookup for an empty value must
        // therefore not fabricate a digest of "" that no stored row can match;
        // signal "not a blind-index match" so _translateQuery falls back to the
        // raw (AEAD-sealed) column, which matches nothing for an empty value — the
        // same unfindable outcome the write produced. Removes the write/read
        // asymmetry (and the latent digest-of-"" collision risk).
