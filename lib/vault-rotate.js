...
var VAULT_SEALED_FILES = [
  ...
  // v1.9.4 — opt-in vault-sealed PEMs. Each is optional (skipped if file
  // doesn't exist, per the {required:false} flag handled by the rotation
  // loop). When present, rotation re-seals them with the new vault keypair
  // so they remain readable after a full vault key rotation. Without this
  // hookup, rotating the vault would leave any sealed PEM unreadable —
  // the failure mode the v1.9.3 spec §7.2 flagged as the dependency.
  ...
  // v1.9.15 — blamejs per-session apiEncrypt server keypair (ML-KEM-1024 + P-384 ECDH).
  // Required from v1.9.15 onward; older databases that pre-date the keypair are
  // backfilled at boot by lib/api-encrypt-keypair.js#loadOrGenerate, so by the
  // time a rotation runs the file always exists.
...
async function rotateDataDirectory(opts) {
  ...
  if (mode === "wrapped") {
    var sealed = await b.vaultWrap.wrap(keysJson, opts.newPassphrase);
