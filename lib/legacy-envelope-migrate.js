...
function makeResealer(keys) {
  return function reseal(legacySealed) {
    ...
    // Idempotency / crash-safety: a value already in the 0xE2 (blamejs) envelope
    // is migrated — return it unchanged instead of feeding it to the 0xE1
    // decrypt, which throws on a non-0xE1 magic byte. Without this guard a run
    // that crashed mid-column-step re-runs on resume (isAlreadyMigrated still
    // sees 0xE1 on the last-migrated db.key.enc), re-encounters a row the prior
    // partial run already rewrote to 0xE2, throws, and bricks boot on every
    // restart. The 0xE2 probe matches isAlreadyMigrated().
    var firstByte = -1;
    try { firstByte = Buffer.from(b64, "base64")[0]; } catch (_e) { firstByte = -1; }
    if (firstByte === 0xE2) return legacySealed;
...
function readMarker(markerPath) {
  var fresh = { stepsDone: [], rowsMigrated: 0, filesMigrated: 0 };
  if (!nodeFs.existsSync(markerPath)) return fresh;
  // b.safeJson.parseOrDefault: a truncated marker from a crashed prior run (or a
  // hand-tampered file) yields the fresh-start object rather than throwing out of
  // the migration and bricking every subsequent boot. The parse also caps size /
  // depth and strips proto-pollution keys.
  var raw = nodeFs.readFileSync(markerPath, "utf8");
  var parsed = b.safeJson.parseOrDefault(raw, fresh, { maxBytes: C.BYTES.mib(1) });
  return (parsed && Array.isArray(parsed.stepsDone)) ? parsed : fresh;
}

function writeMarker(markerPath, state, dryRun) {
  if (dryRun) return;
  b.atomicFile.writeSync(markerPath, JSON.stringify(state, null, 2));
}
...
function migrateFiles(list, state, log, dryRun, reseal) {
  for (var i = 0; i < list.length; i++) {
    ...
    if (dryRun) {
      ...
      b.atomicFile.writeSync(f.path, newSealed, { fileMode: 0o600 });
      log.info("[ok]     " + f.label);
    }
      ...
      for (var ri = 0; ri < allRows.length; ri++) {
        ...
        for (var col in row) {
          ...
          if (v.startsWith(VAULT_PREFIX)) {
            // reseal() decrypts under the current vault key, which throws on a
            // corrupt blob OR on a raw (non-sealed) column whose value merely
            // happens to start with "vault:". Skip + warn on failure rather than
            // aborting the whole migration (which would re-throw on every boot,
            // bricking startup) — this mirrors the fault-tolerance the overflow-
            // JSON branch below already has for the same class of value.
            try {
              updates[col] = reseal(v);
            } catch (resealErr) {
              log.warn("[skip] " + table + "." + col + " (row " + row._id +
                ") — reseal failed, leaving as-is: " + resealErr.message);
            }
            continue;
          }
          ...
          if (col === "data" && (v.charAt(0) === "{" || v.charAt(0) === "[")) {
            try {
              // b.safeJson.parse: caps size/depth and strips proto-pollution keys
              // on the DB overflow blob before we re-seal its nested vault: values.
              var json = b.safeJson.parse(v, { maxBytes: C.BYTES.mib(16) });
              var modified = false;
              for (var k in json) {
