 ...
 *   - server.js boot-time auto-migrate hook (Phase 2+)
 *
 * Independent of lib/vault's swap state. Reads 0xE1 envelopes via the
 * raw `lib/crypto.decrypt` path (which lib/crypto.js still has — it's
 * not swapped until Phase 3) and writes 0xE2 envelopes via blamejs's
 * `b.crypto.encrypt`. Operates on the on-disk format directly with no
 * dependency on lib/vault.seal / unseal — that boundary moves to
 * blamejs in Phase 2 and would refuse 0xE1 once swapped, so the
 * migration cannot route through it.
 *
...
function isAlreadyMigrated(opts) {
  ...
  // generated one. There's nothing to migrate — vault.seal is now
  // post-Phase-2 (0xE2) so the first dbKey lib/db creates is 0xE2,
  ...
  if (!fs.existsSync(dbKeyEncPath)) return true;
...
function makeResealer(keys) {
  return function reseal(legacySealed) {
    ...
    // Decrypt under HermitStash's 0xE1 path.
    var plaintext = hermitstashCrypto.decrypt(b64, keys);
    // Re-encrypt under blamejs's 0xE2 path. b.crypto.encrypt returns
...
function readMarker(markerPath) {
  if (!fs.existsSync(markerPath)) return { stepsDone: [], rowsMigrated: 0, filesMigrated: 0 };
  return JSON.parse(fs.readFileSync(markerPath, "utf8")); // allow:bare-json-parse — parsing migration marker we wrote ourselves in this same module
}

function writeMarker(markerPath, state, dryRun) {
  if (dryRun) return;
  fs.writeFileSync(markerPath + ".tmp", JSON.stringify(state, null, 2));
...
function run(opts) {
  ...
  var PATHS = C.PATHS;
  var markerPath = path.join(PATHS.DATA_DIR, "envelope-migration.marker");
