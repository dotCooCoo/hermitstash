...
function parseMultipart(req, maxSize) {
  return accumulateBody(req, maxSize).then((buf) => {
    ...
    while (pos < buf.length) {
      const start = bufIndexOf(buf, sep, pos);
      if (start === -1) break;
      const nextStart = bufIndexOf(buf, sep, start + sep.length + 2);
      ...
      const headerEnd = bufIndexOf(partBuf, Buffer.from("\r\n\r\n"), 0);
...
function bufIndexOf(buf, needle, start) {
