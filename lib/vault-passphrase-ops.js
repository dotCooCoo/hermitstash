...
async function sealVaultKey(passphrase, opts) {
  ...
  var plaintextBytes = nodeFs.readFileSync(PLAINTEXT_PATH);
  var sealedBytes = await b.vaultWrap.wrap(plaintextBytes, passphrase);

  // Step 1: write .tmp + fsync
  nodeFs.writeFileSync(SEALED_TMP_PATH, sealedBytes, { mode: 0o600 });
  _fsyncPath(SEALED_TMP_PATH);
  _fsyncDataDir();

  // Step 2: in-process round-trip verify
  var verifyBytes = nodeFs.readFileSync(SEALED_TMP_PATH);
  var unwrapped;
  try {
  ...
  nodeFs.writeFileSync(markerTmp, JSON.stringify(marker), { mode: 0o600 });
  _fsyncPath(markerTmp);
  nodeFs.renameSync(markerTmp, SETUP_MARKER_PATH);
  _fsyncDataDir();

  // Step 4: atomic rename sealed file into place
  nodeFs.renameSync(SEALED_TMP_PATH, SEALED_PATH);
  _fsyncDataDir();
...
async function unsealVaultKey(passphrase) {
  ...
  nodeFs.writeFileSync(PLAINTEXT_TMP_PATH, plaintextBytes, { mode: 0o600 });
  _fsyncPath(PLAINTEXT_TMP_PATH);
  _fsyncDataDir();

  // Step 2: round-trip sanity — re-read tmp, verify bytes match
  var verifyBytes = nodeFs.readFileSync(PLAINTEXT_TMP_PATH);
  ...
  nodeFs.writeFileSync(markerTmp, JSON.stringify(marker), { mode: 0o600 });
  _fsyncPath(markerTmp);
  nodeFs.renameSync(markerTmp, REMOVE_MARKER_PATH);
  _fsyncDataDir();

  // Step 4: atomic rename plaintext into place
  nodeFs.renameSync(PLAINTEXT_TMP_PATH, PLAINTEXT_PATH);
  _fsyncDataDir();
