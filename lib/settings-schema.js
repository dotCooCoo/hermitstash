// codebase-patterns:allow-file raw-byte-literal — schema declares operator-input maxLen / min / max bounds; settings layer mirrors config defaults
// codebase-patterns:allow-file raw-time-literal — same; schema bounds collide with multiples-of-60 (sessionIdleTimeout / uploadTimeout)
/**
...
function isValidUrl(v) {
  ...
  // ALLOW_HTTP_ALL: settings-schema gates http:// and https:// equally; the
  // soft "rpOrigin is not HTTPS" startup warning is the operator nudge.
  // safeUrl's default allowlist is https-only, which would silently drop
  // http://umbrel-dev.local:3080 etc. and fall back to a code default.
  try {
    var u = b.safeUrl.parse(v, { allowedProtocols: b.safeUrl.ALLOW_HTTP_ALL });
...
var SCHEMA = {
  ...
  sessionIdleTimeout: { type: "number", min: b.constants.TIME.minutes(1), max: b.constants.TIME.days(1) },
  maxFileSize:        { type: "number", min: 0, max: b.constants.BYTES.gib(10) },
  uploadTimeout:      { type: "number", min: b.constants.TIME.seconds(10), max: b.constants.TIME.hours(1) },
  ...
  publicMaxBundleSize:{ type: "number", min: 0, max: b.constants.BYTES.gib(10) },
  publicIpQuotaBytes: { type: "number", min: 0 },
  auditRetentionDays: { type: "number", min: 0, max: 36500 },
  auditIpFull:           { type: "boolean" },
  auditCaptureUserAgent: { type: "boolean" },
  auditChainEnabled:          { type: "boolean" },
  auditArchiveEnabled:        { type: "boolean" },
  siemEnabled:                { type: "boolean" },
  siemProtocol:               { type: "enum", values: ["syslog", "webhook"] },
  siemUrl:                    { type: "string", maxLen: 1000 },
  siemWebhookAuth:            { type: "enum", values: ["none", "bearer", "basic", "header"] },
  siemWebhookToken:           { type: "credential", maxLen: 2000 },
  siemMinLevel:               { type: "enum", values: ["info", "warn", "error"] },
  auditArchiveThresholdRows:  { type: "number", min: 1000, max: 100000000 },
  auditArchivePassphrase:     { type: "credential", maxLen: 500 },
  ...
  backupSchedule:     { type: "number", min: b.constants.TIME.hours(1), max: b.constants.TIME.days(7) },
  backupTimeOfDay:    { type: "string", maxLen: 5 },
  backupTimezone:     { type: "timezone", maxLen: 50 },
  ...
  enforceMtls:        { type: "boolean" },
  emailEnabled:       { type: "boolean" },
  // TODO(email-backends): coupled to lib/email.js sendEmail() dispatch and the
  // <select> in views/admin.html (#pane-email). Adding a 3rd backend triples
  // the valid combinations; refactor all three sites to share a SUPPORTED_BACKENDS
  // list + parseBackendChain helper before extending.
  emailBackend:       { type: "enum", values: ["smtp", "resend", "smtp+resend", "resend+smtp"] },
  backupScope:        { type: "enum", values: ["db", "full"] },
  emailTemplateMode:  { type: "enum", values: ["text", "html"] },

  // Tailscale
  tailscaleEnabled:            { type: "boolean" },
  tailscaleSocket:             { type: "path", maxLen: 500 },
  tailscaleSso:                { type: "boolean" },
  tailscaleSsoGrant:           { type: "string", maxLen: 200 },
  tailscaleSsoAllowlist:       { type: "list", maxLen: 5000 },
  tailscaleHostnameAutoConfig: { type: "boolean" },
  tailscaleServeMode:          { type: "enum", values: ["off", "serve", "funnel"] },
  ...
  additionalOrigins:  { type: "list", maxLen: 5000 },
  adminAllowedCidrs:  { type: "list", maxLen: 2000 },

  // Paths
  uploadDir:          { type: "path", maxLen: 500 },
  chunkScratchDir:    { type: "path", maxLen: 500 },
...
function sanitize(key, value) {
  // Own-property lookup so an inherited Object.prototype member name doesn't
  // resolve to a truthy function and skip the unknown-key fallback.
  var schema = Object.prototype.hasOwnProperty.call(SCHEMA, key) ? SCHEMA[key] : null;
...
function validate(key, value) {
  // Own-property lookup (see sanitize) so an inherited member name can't pass as
  // a known schema entry.
  var schema = Object.prototype.hasOwnProperty.call(SCHEMA, key) ? SCHEMA[key] : null;
  ...
  switch (schema.type) {
    case "number": {
      if (value === "") return { valid: true }; // empty = use default
    ...
    case "timezone": {
      if (value === "") return { valid: true }; // empty = use default
      // Intl.DateTimeFormat throws on invalid IANA names. The browser-side
      // dropdown constrains choices but the env var path doesn't, so the
      // server is the source of truth for "is this a real timezone."
      try {
        new Intl.DateTimeFormat("en-US", { timeZone: value }).format(new Date());
        return { valid: true };
      } catch (_e) {
        return { valid: false, error: "not a valid IANA timezone (e.g. UTC, America/New_York, Europe/London)" };
      }
    }
    default:
      return { valid: true };
